...
export async function autoRetry (
  ...
  while (maxRetries-- > 0) {
    try {
      ...
      const errorLog = `${
        fatal ? 'FAIL' : 'WARN'
      }: (${maxRetries} retries remaining): ${String(error)}`
      log(
        ns,
        errorLog,
        fatal,
        !verbose ? undefined : fatal ? 'error' : 'warning'
      )
...
export function checkNsInstance (ns, fnName = 'this function') {
  if (!ns.print)
    throw `The first argument to ${fnName} should be a 'ns' instance.`
  return ns
}
