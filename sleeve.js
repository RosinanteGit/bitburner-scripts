...
export async function main (ns) {
  ...
  let numSleeves
  try {
    numSleeves = await getNsDataThroughFile(
      ns,
      `ns.sleeve.getNumSleeves()`,
      '/Temp/sleeve-count.txt'
    )
  } catch {
    return ns.print(
      'User does not appear to have access to sleeves. Exiting...'
    )
  }
  for (let i = 0; i < numSleeves; i++) availableAugs[i] = null

  while (true) {
    let cash =
      ns.getServerMoneyAvailable('home') - Number(ns.read('reserve.txt'))
    let budget = cash * options['aug-budget']
    let playerInfo = await getNsDataThroughFile(
      ns,
      'ns.getPlayer()',
    ...
    for (let i = 0; i < numSleeves; i++) {
      ...
      if (shock == 0 && availableAugs[i] == null)
        // No augs are available augs until shock is 0
        availableAugs[i] = (
          await getNsDataThroughFile(
            ns,
            `ns.sleeve.getSleevePurchasableAugs(${i})`,
            '/Temp/sleeve-augs.txt'
          )
        ).sort((a, b) => a.cost - b.cost) // list of { name, cost }
      if (shock == 0 && availableAugs[i].length > 0) {
        const cooldownLeft = Math.max(
          0,
          options['buy-cooldown'] - (Date.now() - (lastPurchase[i] || 0))
        )
        const [batchCount, batchCost] = availableAugs[i].reduce(
          ([n, c], aug) =>
            c + aug.cost <= budget ? [n + 1, c + aug.cost] : [n, c],
          [0, 0]
        )
        const purchaseUpdate =
          `sleeve ${i} can afford ${batchCount
            .toFixed(0)
            .padStart(2)}/${availableAugs[i].length
            .toFixed(0)
            .padEnd(2)} remaining augs (cost ${formatMoney(batchCost)} of ` +
          `${formatMoney(
            availableAugs[i].reduce((t, aug) => t + aug.cost, 0)
          )}).`
        if (lastUpdate[i] != purchaseUpdate)
          log(
            ns,
            `INFO: With budget ${formatMoney(budget)}, ` +
              (lastUpdate[i] = purchaseUpdate) +
              ` (Min batch size: ${
                options['min-aug-batch']
              }, Cooldown: ${formatDuration(cooldownLeft)})`
          )
        if (
          cooldownLeft == 0 &&
          batchCount > 0 &&
          (batchCount >= availableAugs[i].length - 1 ||
            batchCount >= options['min-aug-batch'])
        ) {
          // Don't require the last aug it's so much more expensive
          let strAction = `Purchase ${batchCount} augmentations for sleeve ${i} at total cost of ${formatMoney(
            batchCost
          )}`
          ...
          if (
            await getNsDataThroughFile(
              ns,
              JSON.stringify(toPurchase.map(a => a.name)) +
                `.reduce((s, aug) => s && ns.sleeve.purchaseSleeveAug(${i}, aug), true)`,
      ...
      if (sync < 100) {
        ...
        if (
          ...
          Date.now() - (lastUpdate[i] ?? 0) > minTaskWorkTime
        ) {
        ...
        shock > 0 &&
        options['shock-recovery'] > 0 &&
        Math.random() < options['shock-recovery']
      ) {
        // Recover from shock
        ...
        if (
          ...
          log(
            ns,
            `INFO: Sleeve ${i} is recovering from shock... ${shock.toFixed(2)}%`
          )
          lastUpdate[i] = Date.now()
        }
      } else if (
        i == 0 &&
        playerInfo.isWorking &&
        playerInfo.workType == 'Working for Faction'
      ) {
        // If player is currently working for faction rep, sleeves 0 shall help him out (only one sleeve can work for a faction)
        ...
        i == 0 &&
        playerInfo.isWorking &&
        playerInfo.workType == 'Working for Company'
      ) {
        // If player is currently working for a company rep, sleeves 0 shall help him out (only one sleeve can work for a company)
        designatedTask = `work for company '${playerInfo.companyName}'`
        command = `ns.sleeve.setToCompanyWork(${i}, '${playerInfo.companyName}')`
      } else {
        // Do something productive
        let crime =
          options.crime || (sleeveStats.strength < 100 ? 'mug' : 'homicide')
        designatedTask = `commit ${crime}`
        command = `ns.sleeve.setToCommitCrime(${i}, '${crime}')`
      }
      // Don't change tasks if we've changed tasks recently
      if (
        Date.now() - (lastReassign[i] || 0) < minTaskWorkTime ||
        task[i] == designatedTask
      )
        continue
      ...
      if (await getNsDataThroughFile(ns, command, tempFile)) {
        ...
        if (designatedTask.startsWith('work for faction')) {
          log(
            ns,
            `WARN: Failed to ${strAction} - work type may not be supported.`,
            'warning'
          )
          workByFaction[playerInfo.currentWorkFactionName] =
            (workByFaction[playerInfo.currentWorkFactionName] || 0) + 1
        } else log(ns, `ERROR: Failed to ${strAction}`, 'error')
      }
    }
