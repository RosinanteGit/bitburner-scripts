...
export async function main (ns) {
  ...
  if (sleepDuration > 0) await ns.sleep(sleepDuration)
  do {
    if (
      !(await ns.hack(ns.args[0], { stock: manipulateStock })) &&
      !disableToastWarnings
    )
      ns.toast(
        `Warning, hack stole 0 money. Might be a misfire. ${JSON.stringify(
          ns.args
        )}`,
        'warning'
      )
