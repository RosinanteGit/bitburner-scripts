...
const jobs = [
  // Job stat requirements for a company with a base stat modifier of +224 (modifier of all megacorps except the ones above which are 25 higher)
  {
    name: 'it',
...
const factionSpecificConfigs = [{ name: 'Slum Snakes', forceUnlock: true }]
const factions = [
  'Illuminati',
  'Daedalus',
  'The Covenant',
...
const preferredCrimeFactionOrder = [
  'Netburners',
  'Slum Snakes',
  'NiteSec',
  'Tetrads',
...
const argsSchema = [
  ...
  ], // Factions will be removed from our 'early-faction-order' once all augs with these stats have been bought out
...
export async function main (ns) {
  ...
  const firstFactions = (options.first = (options.first || []).map(f =>
    f.replaceAll('_', ' ')
  ))
  let skipFactionsConfig = (options.skip = (options.skip || []).map(f =>
    f.replaceAll('_', ' ')
  ))
  ...
  if (crimeFocus && noFocus)
    return ns.tprint(
      'ERROR: Cannot use --no-focus and --crime-focus at the same time. You need to focus to do crime!'
    )
  ...
  if (skipFactionsConfig.length > 0)
    ns.print(`--skip factions: ${skipFactionsConfig.join(', ')}`)
  if (desiredAugStats.length > 0)
  ...
  if (2 in dictSourceFiles) {
    ...
    if (gangInfo && gangInfo.faction) {
      ...
      let configGangIndex = preferredEarlyFactionOrder.findIndex(
        f => f === 'Slum Snakes'
      )
      ...
      allGangFactions =
        (await getNsDataThroughFile(
          ns,
          'Object.keys(ns.gang.getOtherGangInformation())',
          '/Temp/gang-names.txt'
        )) || []
    }
  }
  ...
  const dictFactionAugs = await getNsDataThroughFile(
    ns,
    dictCommand(factions, 'ns.getAugmentationsFromFaction(o)'),
  ...
  const dictAugRepReqs = await getNsDataThroughFile(
    ns,
    dictCommand(augmentationNames, 'ns.getAugmentationRepReq(o)'),
    '/Temp/aug-repreqs.txt'
  )
  const dictAugStats = await getNsDataThroughFile(
    ns,
    dictCommand(augmentationNames, 'ns.getAugmentationStats(o)'),
    '/Temp/aug-stats.txt'
  )

  ownedAugmentations = await getNsDataThroughFile(
    ns,
    `ns.getOwnedAugmentations(true)`,
  ...
  if (shouldFocusAtWork) {
    // Check if we have an augmentation that lets us not have to focus at work (always nicer if we can background it)
    let activeAugmentations = await getNsDataThroughFile(
      ns,
      `ns.getOwnedAugmentations()`,
      '/Temp/player-augs-installed.txt'
    )
    shouldFocusAtWork = !activeAugmentations.includes(
      'Neuroreceptor Management Implant'
    )
  }

  mostExpensiveAugByFaction = Object.fromEntries(
    factions.map(f => [
      f,
      dictFactionAugs[f]
          ...
          aug =>
            ...
            (Object.keys(dictAugStats[aug]).length == 0 ||
              ...
              Object.keys(dictAugStats[aug]).some(key =>
                desiredAugStats.some(stat => key.includes(stat))
              ))
        )
        .reduce((max, aug) => Math.max(max, dictAugRepReqs[aug]), -1)
    ])
  ...
  let completedFactions = Object.keys(mostExpensiveAugByFaction).filter(
    fac =>
      mostExpensiveAugByFaction[fac] == -1 &&
      !factionSpecificConfigs.find(c => c.name == fac)?.forceUnlock
  )
  let skipFactions = skipFactionsConfig.concat(completedFactions)
  let softCompletedFactions = Object.keys(
  ...
  if (completedFactions.length > 0)
    ns.print(
      `${
        completedFactions.length
      } factions are completed (all augs purchased): ${completedFactions.join(
        ', '
      )}`
    )
  ...
  while (true) {
    ...
    const factionWorkOrder = firstFactions
      .concat(
        priorityFactions.filter(
          f => !firstFactions.includes(f) && !skipFactions.includes(f)
        )
      )
    ...
    for (const faction of factionWorkOrder) {
      ...
      if (preferredCompanyFactionOrder.includes(faction))
        ...
        earnedNewFactionInvite = await workForMegacorpFactionInvite(
          ns,
          faction,
          true
        )
    ...
    let megacorpFactions = preferredCompanyFactionOrder.filter(
      f => !skipFactions.includes(f)
    )
    ...
    let allIncompleteFactions = factions
      .concat(joinedFactions.filter(f => !factions.includes(f)))
      .filter(f => !skipFactions.includes(f) && !completedFactions.includes(f))
      .sort(
        (a, b) => mostExpensiveAugByFaction[a] - mostExpensiveAugByFaction[b]
      )
    ...
    for (const faction of allIncompleteFactions.filter(
      f => !softCompletedFactions.includes(f)
    ))
      await workForSingleFaction(ns, faction)
    if (scope < 5) continue
    ...
    if (noCrime) {
      ns.print(
        `--no-crime (or --no-focus): Crimes are disabled, so sleeping for a while (30s) then checking back on whether there's any work to be done...`
      )
...
const dictCommand = (list, command) =>
  `Object.fromEntries(${JSON.stringify(list)}.map(o => [o, ${command}]))`
...
const requiredKillsByFaction = {
  'Speakers for the Dead': 30,
  'The Dark Army': 5
}
...
async function earnFactionInvite (ns, factionName) {
  ...
  if (
    (['Aevum', 'Sector-12'].includes(factionName) &&
      (precludingFaction = [
        ...
        'Ishima',
        'Volhaven'
      ].find(f => joinedFactions.includes(f)))) ||
    (['Chongqing', 'New Tokyo', 'Ishima'].includes(factionName) &&
      (precludingFaction = ['Aevum', 'Sector-12', 'Volhaven'].find(f =>
    ...
    return ns.print(
      `${reasonPrefix} precluding faction "${precludingFaction}"" has been joined.`
    )
  ...
  let requirement
  if (
    (requirement = requiredMoneyByFaction[factionName]) &&
    player.money < requirement
  )
    return ns.print(
      `${reasonPrefix} you have insufficient money. Need: ${formatMoney(
        requirement
      )}, Have: ${formatMoney(player.money)}`
    )
  if (
    ...
    return ns.print(
      `${reasonPrefix} you have insufficient hack level. Need: ${requirement}, Have: ${player.hacking}`
    )
  ...
  let doCrime = false
  if (
    (requirement = requiredKarmaByFaction[factionName]) &&
    -ns.heart.break() < requirement
  ) {
    ns.print(
      `${reasonPrefix} you have insufficient Karma. Need: ${-requirement}, Have: ${ns.heart.break()}`
    )
    doCrime = true
  }
  if (
    (requirement = requiredKillsByFaction[factionName]) &&
    player.numPeopleKilled < requirement
  ) {
    ns.print(
      `${reasonPrefix} you have insufficient kills. Need: ${requirement}, Have: ${player.numPeopleKilled}`
    )
    doCrime = true
  }
  let deficientStats
  if (
    ...
    !(
      reqHackingOrCombat.includes(factionName) &&
      player.hacking >= requiredHackByFaction[factionName]
    )
  ) {
    ...
    if (
      ...
      return ns.print(
        'Physical mults / exp_mults are too low to increase stats in a reasonable amount of time'
      )
  ...
  if (doCrime && noCrime)
    return ns.print(
      `--no-crime (or --no-focus): Doing crime to meet faction requirements is disabled.`
    )
  if (doCrime)
  ...
  else return await tryJoinFaction(ns, factionName)
}
...
export async function crimeForKillsKarmaStats (
  ...
  let strRequirements = []
  let forever =
    reqKills >= Number.MAX_SAFE_INTEGER ||
    reqKarma >= Number.MAX_SAFE_INTEGER ||
    reqStats >= Number.MAX_SAFE_INTEGER
  if (reqKills)
    strRequirements.push(
      () => `${reqKills} kills (Have ${player.numPeopleKilled})`
    )
  if (reqKarma)
    strRequirements.push(() => `-${reqKarma} Karma (Have ${ns.heart.break()})`)
  if (reqStats)
    strRequirements.push(
      () =>
  ...
  while (
    ...
    let crimeChances = await getNsDataThroughFile(
      ...
      `Object.fromEntries(${JSON.stringify(
        bestCrimesByDifficulty
      )}.map(c => [c, ns.getCrimeChance(c)]))`,
    ...
    crime =
      (karma < 1 || player.strength < 10) && crimeCount < 10
        ? 'mug'
        : karma < 5 &&
          player.strength > 20 &&
          crimeCount < 20 &&
          crimeChances[2] > 0.5
        ? 'homicide' // Start with a few fast crimes to boost stats / crime chances if we haven't done much crime before
        ...
        : bestCrimesByDifficulty.find((c, index) =>
            doFastCrimesOnly
              ? index > 1
              : crimeChances[c] >= chanceThresholds[index]
          ) // Otherwise, crime based on success chance vs relative reward (precomputed)
    if (
      lastCrime != crime ||
      Date.now() - lastStatusUpdateTime > statusUpdateInterval
    ) {
      ns.print(
        `Committing "${crime}" (${(100 * crimeChances[crime]).toPrecision(
          3
        )}% success) ` +
          (forever
            ? 'forever...'
            : `until we reach ${strRequirements.map(r => r()).join(', ')}`)
      )
      lastCrime = crime
    ...
    while (
      (player = ns.getPlayer()).crimeType == `commit ${crime}` ||
      player.crimeType == crime // If we woke up too early, wait a little longer for the crime to finish
    )
      await ns.sleep(10)
    crimeCount++
  }
  ns.print(
    `Done committing crimes. Reached ${strRequirements
      .map(r => r())
      .join(', ')}`
  )
  return true
...
async function studyForCharisma (ns) {
  ...
  if (
    ...
    announce(
      ns,
      `Started studying 'Leadership' at 'ZB Institute Of Technology`,
      'success'
    )
    return true
  }
  announce(
    ns,
    `For some reason, failed to study at university (not in correct city?)`,
    'error'
  )
  return false
}
...
/** @param {NS} ns */
export async function waitForFactionInvite (
  ns,
  factionName,
  maxWaitTime = 20000
) {
  ...
  do {
    ...
    if (
      invitations.includes(factionName) ||
      joinedFactions.includes(factionName)
    )
      break
...
async function getCurrentFactionFavour (ns, factionName) {
  return await getNsDataThroughFile(
    ns,
    `ns.getFactionFavor('${factionName}')`,
...
export async function workForSingleFaction (
  ...
  const forceUnlock =
    factionConfig?.forceUnlock || options.first.includes(factionName)
  ...
  let favorRepRequired = Math.max(
    0,
    repToFavour(repToDonate) - repToFavour(startingFavor)
  )
  ...
  let factionRepRequired = forceUnlockDonations
    ? favorRepRequired
    : Math.min(highestRepAug, favorRepRequired)
  if (highestRepAug == -1 && !forceUnlock)
    return ns.print(
      `All "${factionName}" augmentations are owned. Skipping unlocking faction...`
    )
  // Ensure we get an invite to location-based factions we might want / need
  if (!(await earnFactionInvite(ns, factionName)))
    return ns.print(
      `We are not yet part of faction "${factionName}". Skipping working for faction...`
    )
  if (startingFavor >= repToDonate)
    // If we have already got 150+ favor, we've unlocked donations - no need to grind for rep
    return ns.print(
  ...
  if (factionName === playerGang || allGangFactions.includes(factionName))
    return ns.print(
      `"${factionName}" is an active gang faction. Cannot work for gang factions...`
    )
  if (
    forceUnlockDonations &&
    mostExpensiveAugByFaction[factionName] < 0.2 * factionRepRequired
  ) {
    // Special check to avoid pointless donation unlocking
    ns.print(
      `The last "${factionName}" aug is only ${mostExpensiveAugByFaction[
        factionName
      ].toLocaleString()} rep, ` +
  ...
  if (
    forceBestAug ||
    highestRepAug <= 1.1 * Math.max(currentReputation, factionRepRequired)
  ) {
    forceBestAug = true
  ...
  if (currentReputation >= factionRepRequired)
    return ns.print(
      `Faction "${factionName}" required rep of ${factionRepRequired.toLocaleString()} has already been attained ` +
        `(Current rep: ${Math.round(
          currentReputation
        ).toLocaleString()}). Skipping working for faction...`
    )

  ns.print(
    `Faction "${factionName}" Highest Aug Req: ${highestRepAug.toLocaleString()}, Current Favor (${startingFavor}/${repToDonate}) Req: ${favorRepRequired.toLocaleString()}`
  )
  ...
  if (prioritizeInvites && !forceUnlockDonations && !forceBestAug)
    return ns.print(
      `--prioritize-invites Skipping working for faction for now...`
    )

  let lastStatusUpdateTime = 0
  while (
    (currentReputation = ns.getFactionRep(factionName)) < factionRepRequired
  ) {
    ...
    if (
      await getNsDataThroughFile(
        ns,
        `ns.workForFaction('${factionName}', '${factionWork}',  ${shouldFocusAtWork})`,
    ...
    if (
      ...
      const eta_milliseconds =
        (factionRepRequired - currentReputation) /
        (ns.getPlayer().workRepGainRate /
          200) /* workRepGainRate is a rate per game-tick (200ms) */
      ns.print(
        (lastFactionWorkStatus = status) +
          ` Currently at ${Math.round(currentReputation).toLocaleString()}, ` +
          `earning ${formatNumberShort(
            ns.getPlayer().workRepGainRate * 5
          )} rep/sec. (ETA: ${formatDuration(eta_milliseconds)})`
      )
      lastStatusUpdateTime = Date.now()
    }
    ...
    if (currentFavor > startingFavor && !forceBestAug) {
      favorRepRequired = Math.max(
        0,
        repToFavour(repToDonate) - repToFavour(startingFavor)
      )
      factionRepRequired = forceUnlockDonations
        ? favorRepRequired
        : Math.min(highestRepAug, favorRepRequired)
    }
...
async function detectBestFactionWork (ns, factionName) {
  ...
  for (const work of ['security', 'field', 'hacking']) {
    if (
      !(await getNsDataThroughFile(
        ns,
        `ns.workForFaction('${factionName}', '${work}',  ${shouldFocusAtWork})`,
        '/Temp/work-for-faction.txt'
      ))
    ) {
...
export async function workForAllMegacorps (
  ns,
  megacorpFactionsInPreferredOrder,
  alsoWorkForCompanyFactions,
  oneCompanyFactionAtATime
) {
  let player = ns.getPlayer()
  if (player.hacking < 225)
    return ns.print(
      `Hacking Skill ${player.hacking} is to low to work for any megacorps (min req. 225).`
    )
  let joinedCompanyFactions = player.factions.filter(f =>
    megacorpFactionsInPreferredOrder.includes(f)
  ) // Company factions we've already joined
  if (joinedCompanyFactions.length > 0)
    ns.print(
      `${
        joinedCompanyFactions.length
      } companies' factions have already been joined: ${joinedCompanyFactions.join(
        ', '
      )}`
    )
  ...
  for (const factionName of megacorpFactionsInPreferredOrder) {
    if (
      (await workForMegacorpFactionInvite(ns, factionName, doFactionWork)) &&
      doFactionWork
    )
  ...
  if (alsoWorkForCompanyFactions && !oneCompanyFactionAtATime) {
    ...
    ns.print(
      `Done working for companies, now working for all incomplete company factions...`
    )
    for (const factionName of megacorpFactionsInPreferredOrder)
      await workForSingleFaction(ns, factionName)
  }
...
export async function tryBuyReputation (ns) {
  ...
  if (ns.getPlayer().money > 100e9) {
    // If we're wealthy, hashes have relatively little monetary value, spend hacknet-node hashes on contracts to gain rep faster
    let spentHashes = await getNsDataThroughFile(
      ns,
      'ns.hacknet.numHashes() + ns.hacknet.spendHashes("Generate Coding Contract") - ns.hacknet.numHashes()',
      '/Temp/spend-hacknet-hashes.txt'
    )
    if (spentHashes > 0) {
      announce(
        ns,
        `Generated a new coding contract for ${formatNumberShort(
          Math.round(spentHashes / 100) * 100
        )} hashes`,
        'success'
      )
    }
...
/** @param {NS} ns */
export async function workForMegacorpFactionInvite (
  ns,
  factionName,
  waitForInvite
) {
  ...
  if (itJob.reqHack[0] + statModifier > player.hacking)
    ...
    return ns.print(
      `Cannot yet work for "${companyName}": Need Hack ${itJob.reqHack[0] +
        statModifier} to get hired (current Hack: ${player.hacking});`
    )
  ...
  let studying = false,
    ...
    backdoored = false
  while (
    (currentReputation = ns.getCompanyRep(companyName)) <
      repRequiredForFaction &&
    ...
    if (currentJobTier < bestJobTier || currentRole != bestRoleName) {
      ...
      if (
        await getNsDataThroughFile(
          ns,
          `ns.applyToCompany('${companyName}','${bestRoleName}')`,
          '/Temp/apply-to-company.txt'
        )
      )
        announce(
          ns,
          `Successfully applied to "${companyName}" for a '${bestRoleName}' Job or Promotion`,
          'success'
        )
        ...
        announce(
          ns,
          `Application to "${companyName}" for a '${bestRoleName}' Job or Promotion failed.`,
          'error'
        )
    ...
    const nextJobTier =
      currentRole == 'it' ? currentJobTier : currentJobTier + 1
    const nextJobName =
      currentRole == 'it' || nextJobTier >= itJob.reqRep.length
        ? 'software'
        : 'it'
    const nextJob = nextJobName == 'it' ? itJob : softwareJob
    const requiredHack =
      nextJob.reqHack[nextJobTier] === 0
        ? 0
        : nextJob.reqHack[nextJobTier] + statModifier // Stat modifier only applies to non-zero reqs
    const requiredCha =
      nextJob.reqCha[nextJobTier] === 0
        ? 0
        : nextJob.reqCha[nextJobTier] + statModifier // Stat modifier only applies to non-zero reqs
    ...
    if (
      ...
      status =
        `Studying at ZB university until Cha reaches ${requiredCha}...\n` +
        status
      if (
        studying &&
        player.className !== 'taking a Leadership course' &&
        player.className !==
          'Leadership' /* In case className is made more intuitive in the future */
      ) {
        announce(
          ns,
      ...
      if (requiredCha - player.charisma > 10) {
        ...
        let spentHashes = await getNsDataThroughFile(
          ns,
          'ns.hacknet.numHashes() + ns.hacknet.spendHashes("Improve Studying") - ns.hacknet.numHashes()',
          '/Temp/spend-hacknet-hashes.txt'
        )
        if (spentHashes > 0) {
    ...
    if (
      await getNsDataThroughFile(
        ns,
        `ns.applyToCompany('${companyName}','${currentRole}')`,
        '/Temp/apply-to-company.txt'
      )
    )
      announce(
    ...
    if (
      !studying &&
      (!working ||
        Date.now() - lastActionRestart >=
          restartWorkInteval) /* We must periodically restart work to collect Rep Gains */
    ) {
      // Work for the company (assume daemon is grinding hack XP as fast as it can, so no point in studying for that)
      if (
        await getNsDataThroughFile(
          ns,
          `ns.workForCompany('${companyName}',  ${shouldFocusAtWork})`,
          '/Temp/work-for-company.txt'
        )
      ) {
        lastActionRestart = Date.now()
        working = true
        ...
        announce(
          ns,
          `Something went wrong, failed to start working for company "${companyName}".`,
          'error'
        )
        break
      }
    ...
    if (
      ...
      if (!backdoored)
        ...
        backdoored = await getNsDataThroughFile(
          ns,
          `ns.getServer('${serverByCompany[companyName]}').backdoorInstalled`,
          '/Temp/company-is-backdoored.txt'
        )
      const cancellationMult = backdoored ? 0.75 : 0.5 // We will lose some of our gained reputation when we stop working early
      const eta_milliseconds =
        ((requiredRep || repRequiredForFaction) - currentReputation) /
        ((cancellationMult * player.workRepGainRate) /
          200) /* workRepGainRate is a rate per game-tick (200ms) */
      ns.print(
        `Currently a "${player.jobs[companyName]}" ('${currentRole}' #${currentJobTier}) for "${companyName}" ` +
          `earning ${formatNumberShort(
            player.workRepGainRate * 5
          )} rep/sec. (x${cancellationMult} on cancel)\n` +
          `${status}\nCurrent player stats are Hack:${player.hacking} ${
            player.hacking >= (requiredHack || 0) ? '✓' : '✗'
          } ` +
          `Cha:${player.charisma} ${
            player.charisma >= (requiredCha || 0) ? '✓' : '✗'
          } ` +
  ...
  if (currentReputation >= repRequiredForFaction) {
    ns.print(
      `Attained ${repRequiredForFaction.toLocaleString()} rep with "${companyName}".`
    )
