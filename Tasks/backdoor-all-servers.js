...
export let main = async ns => {
  ...
  try {
    ...
    ns.print(
      `${
        hackableServers.filter(s => ns.hasRootAccess(s)).length
      } servers are currently rooted.`
    )
    ns.print(
      `${
        hackableServers.filter(
          s => myHackingLevel > ns.getServerRequiredHackingLevel(s)
        ).length
    ...
    ns.print(
      `${
        hackableServers.filter(
          s =>
            myHackingLevel > ns.getServerRequiredHackingLevel(s) &&
            ns.hasRootAccess(s)
        ).length
    ...
    let toBackdoor = await getNsDataThroughFile(
      ...
      `${JSON.stringify(
        hackableServers
      )}.filter(s => !ns.getServer(s).backdoorInstalled)`,
    ...
    ns.print(
      `${
        toBackdoor.filter(s => ns.hasRootAccess(s)).length
      } of ${count} servers to backdoor are currently rooted.`
    )
    toBackdoor = toBackdoor.filter(
      s => myHackingLevel > ns.getServerRequiredHackingLevel(s)
    )
    ns.print(
      `${toBackdoor.length} of ${count} servers to backdoor are within our hack level (${myHackingLevel}).`
    )
    ...
    for (const server of toBackdoor) {
      ...
      var pid = ns.run(
        getFilePath('/Tasks/backdoor-all-servers.js.backdoor-one.js'),
        1,
        server
      )
      if (pid === 0)
        return ns.print(
          `Couldn't initiate a new backdoor of "${server}"" (insufficient RAM?). Will try again later.`
        )
