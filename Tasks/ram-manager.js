...
export async function main (ns) {
  ...
  if (currentRam >= 2 ** 20)
    return ns.print(
      `We're at max home RAM (2^20 = ${formatNumberShort(currentRam)}GB)`
    )
  const nextRam = currentRam * 2
  const upgradeDesc = `home RAM from ${formatNumberShort(
    currentRam
  )}GB to ${formatNumberShort(nextRam)}GB`
  if (spendable < cost)
    return ns.print(
      `Money we're allowed to spend (${formatMoney(
        spendable
      )}) is less than the cost (${formatMoney(
        cost
      )}) to upgrade ${upgradeDesc}`
    )
  if (ns.upgradeHomeRam()) {
    ...
    if (nextRam != ns.getServerMaxRam('home'))
      announce(
        ns,
        `WARNING: Expected to upgrade ${upgradeDesc}, but new home ram is ${formatNumberShort(
    ...
    announce(
      ns,
      `ERROR: Failed to upgrade ${upgradeDesc} thinking we could afford it (cost: ${formatMoney(
        cost
