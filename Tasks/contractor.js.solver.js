...
export async function main (ns) {
  ...
  for (const contractInfo of contractsDb) {
    ...
    if (answer != null) {
      ...
      if (solvingResult) {
        ns.toast(
          `Solved ${contractInfo.contract} on ${contractInfo.hostname}`,
          'success'
        )
        ns.tprint(
          `Solved ${contractInfo.contract} on ${contractInfo.hostname}. Reward: ${solvingResult}`
        )
      } else {
        ns.tprint(
          `Wrong answer for ${contractInfo.contract} on ${
            contractInfo.hostname
      ...
      ns.tprint(
        `Unable to find the answer for: ${JSON.stringify(contractInfo)}`
      )
    }
    await ns.sleep(10)
  }
