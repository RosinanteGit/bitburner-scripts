...
const argsSchema = [['hide-stocks', false]]
...
export async function main (ns) {
  ...
  if (!options['hide-stocks'] && playerInfo.hasTixApiAccess)
    // Auto-disabled if we do not have the TSK API
    stkSymbols = await getNsDataThroughFile(
      ns,
      `ns.stock.getSymbols()`,
    ...
    try {
      ...
      if (9 in dictSourceFiles) {
        ...
        if (hashes[1] > 0) {
          headers.push('Hashes')
          values.push(
            `${formatNumberShort(hashes[0], 3, 1)}/${formatNumberShort(
              hashes[1],
              3,
              1
            )}`
        ...
        if (
          ns
            ...
            .some(
              p =>
                p.filename.includes('spend-hacknet-hashes') &&
      ...
      if (stkSymbols && !doc.getElementById('stock-display-1')) {
        // Don't add stocks if unavailable or the stockmaster HUD is active
        const stkPortfolio = await getNsDataThroughFile(
          ns,
          JSON.stringify(stkSymbols) +
        ...
        if (stkPortfolio > 0) {
          // Don't bother showing a section for stock if we aren't holding anything
          headers.push('Stock')
