...
function refresh (ns, stocks, myStocks) {
  ...
  stocks.forEach(function (_, i) {
    ...
    if (cost < 0.015 * cashToSpend && stocks[i].shares == 0) {
      ns.print(
        `Removed ${stocks[i].sym} from stocks  Max cost: ${format(cost, true)}`
      )
      stocks.splice(i, 1)
...
function buy (ns, stock, numShares) {
  ...
  ns.print(
    `${actualPrice > 0 ? 'Bought' : 'Failed to buy'} ${format(
      numShares,
      false
    )} shares of ${stock.sym} for ${format(numShares * stock.price, true)}`
  )
}
...
function sell (ns, stock, numShares) {
  ...
  if (ns.stock.sell(stock.sym, numShares) > 0) {
    let message = `Sold   ${format(numShares, false)} shares of ${
      stock.sym
    } for profit of ${format(profit, true)}`
    ns.print(message)
    ...
    ns.print(
      `Failed to sell ${format(numShares, false)} shares of ${
        stock.sym
      } for profit of ${format(profit, true)}`
    )
  }
...
export async function main (ns) {
  ...
  while (true) {
    ...
    let numShares = Math.min(
      Math.floor((cashToSpend - commission) / stocks[0].price),
      maxSharesAvailable
    )
    //buy additional shares if deemed profitable
    if (
      numShares * stocks[0].expRet * stocks[0].price * numCycles >
      commission
    ) {
    ...
    ns.print(
      `Profits: ${format(totalProfit, true)} Losses: ${format(
        totalLosses,
        true
      )}`
    )
