...
export function addEdge (graph, source, destination) {
  ...
  if (
    graph[source].includes(destination) ||
    graph[destination].includes(source)
  ) {
    return graph
  }
  graph = clone(graph)
...
export function findPath (graph, start, end) {
  ...
  while (path[path.length - 1] !== start) {
    path.push(
      Object.keys(depth).find(
        vertex =>
          depth[path[path.length - 1]] - 1 === depth[vertex] &&
          graph[vertex].includes(path[path.length - 1])
      )
    )
  }
  return path.reverse()
