...
export async function main (ns) {
  ...
  const spendOnServer =
    options['spend-on-server']?.replaceAll('_', ' ') ?? undefined
  disableLogs(ns, ['sleep'])
  ns.print(
    `Starting spend-hacknet-hashes.js to ensure no hashes go unspent. Will check in every ${formatDuration(
      interval
    )}`
  )
  ns.print(
    liquidate
      ? `-l --liquidate mode active! Will spend all hashes on money as soon as possible.`
      : `Only spending hashes every when near capacity to avoid wasting them.`
  )
  if (ns.hacknet.hashCapacity() == 0)
    return ns.print(
      'We have hacknet nodes, not hacknet servers, so spending hashes is not applicable.'
    )

  while (true) {
    let capacity = ns.hacknet.hashCapacity() || Number.MAX_VALUE
    let startingHashes = ns.hacknet.numHashes() || 0
    let globalProduction = Array.from(
    ...
    while (
      success &&
      ns.hacknet.numHashes() > (liquidate ? 4 : capacity - reserve)
    )
      success = ns.hacknet.spendHashes(toBuy, spendOnServer)
    if (!success)
      ns.print(
        `Weird, failed to spend hashes. (Have: ${ns.hacknet.numHashes()} Capacity: ${ns.hacknet.hashCapacity()}`
      )
    if (verbose && ns.hacknet.numHashes() < startingHashes)
      ns.print(
        `Spent ${(startingHashes - ns.hacknet.numHashes()).toFixed(0)} hashes` +
          (liquidate ? '' : ` to avoid reaching capacity (${capacity})`) +
          ` at ${globalProduction.toPrecision(3)} hashes per second`
      )
