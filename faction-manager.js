...
const argsSchema = [
  ...
  ], // Augs that give these will be starred
...
export function autocomplete (data, args) {
  ...
  if (lastFlag == '--sort') return stat_multis
...
export async function main (ns) {
  ...
  const afterFactions = options['after-faction'].map(f =>
    f.replaceAll('_', ' ')
  )
  const omitFactions = options['disable-faction'].map(f =>
    f.replaceAll('_', ' ')
  )
  ...
  playerData = await getNsDataThroughFile(
    ns,
    'ns.getPlayer()',
  ...
  if (sf4Level == 0)
    log(
      ns,
      `WARNING: This script makes heavy use of singularity functions. Without SF4, you're unlikely to get it working.`
    )
  else if (sf4Level < 3)
    log(
      ns,
  ...
  log(
    ns,
    `Player has sf11Level ${sf11Level}, so the multiplier after each aug purchased is ${augCountMult}.`
  )
  ...
  displayFactionSummary(
    ns,
    verbose,
    sort,
    options.u || options.unique,
    afterFactions,
    options['hide-stat']
  )
...
async function updateFactionData (ns, allFactions, factionsToOmit) {
  ...
  factionNames.push(
    ...joinedFactions.filter(
      f => !factionNames.includes(f) && !factionsToOmit.includes(f)
    )
  )
  ...
  let factionsDictCommand = command =>
    `Object.fromEntries(${JSON.stringify(
      factionNames
    )}.map(faction => [faction, ${command}]))`
  let dictFactionAugs = await getNsDataThroughFile(
    ns,
  ...
  const gangFaction = await getNsDataThroughFile(
    ns,
    'ns.gang.inGang() ? ns.gang.getGangInformation().faction : false',
    '/Temp/gang-faction.txt'
  )
  if (gangFaction)
    dictFactionAugs[gangFaction] = dictFactionAugs[gangFaction]?.filter(
      a => a != 'The Red Pill'
    )
  ...
  factionData = Object.fromEntries(
    factionNames.map(faction => [
      faction,
      {
        ...
        donationsUnlocked:
          dictFactionFavors[faction] >= ns.getFavorToDonate() &&
          faction !== gangFaction, // Can't donate to gang factions for rep
        ...
        totalUnownedMults: function () {
          return this.unownedAugmentations()
            ...
            .reduce(
              (arr, aug) =>
                Object.keys(aug.stats).forEach(
                  stat => (arr[stat] = (arr[stat] || 1) * aug.stats[stat])
                ) || arr,
              new Map()
            )
        }
...
async function updateAugmentationData (ns, desiredStatsFilters, desiredAugs) {
  ...
  const dictAugRepReqs = await getNsDataThroughFile(
    ns,
    augsDictCommand('ns.getAugmentationRepReq(aug)'),
    '/Temp/aug-repreqs.txt'
  )
  const dictAugPrices = await getNsDataThroughFile(
    ns,
    augsDictCommand('ns.getAugmentationPrice(aug)'),
    '/Temp/aug-prices.txt'
  )
  const dictAugStats = await getNsDataThroughFile(
    ns,
    augsDictCommand('ns.getAugmentationStats(aug)'),
    '/Temp/aug-stats.txt'
  )
  const dictAugPrereqs = await getNsDataThroughFile(
    ns,
    augsDictCommand('ns.getAugmentationPrereq(aug)'),
    '/Temp/aug-prereqs.txt'
  )
        ...
        getFromJoined: function () {
          return (
            ...
            this.joinedFactionsWithAug()
              ...
              .sort(
                (a, b) =>
                  getReqDonationForAug(this, a) - getReqDonationForAug(this, b)
              )[0] ||
        ...
        toString: function () {
          ...
          const statsString =
            ...
            (statKeys.length == 0
              ? ''
              : ` { ${statKeys
                  .map(prop => shorten(prop) + ': ' + this.stats[prop])
                  .join(', ')} }`)
          const factionName = this.getFromJoined() || this.getFromAny
          const fCreep = Math.max(0, factionName.length - factionColWidth)
          const augNameShort =
            this.name.length <= augColWidth - fCreep
              ? this.name
              : `${this.name.slice(
                  0,
                  Math.ceil(augColWidth / 2 - 3 - fCreep)
                )}...${this.name.slice(
                  this.name.length - Math.floor(augColWidth / 2)
                )}`
          return (
            `${this.desired ? '*' : ' '} ${
              this.canAfford() ? '✓' : this.canAffordWithDonation() ? '$' : '✗'
            } Price: ${formatMoney(this.price, 4).padEnd(7)}  ` +
            `Rep: ${formatNumberShort(
              this.reputation,
              4
            )}  Faction: ${factionName.padEnd(
              factionColWidth
            )}  Aug: ${augNameShort.padEnd(augColWidth - fCreep)}` +
            `  ${
              statsString.length <= statsColWidth
                ? statsString
  ...
  let propagateDesired = aug =>
    !aug.desired || !aug.prereqs
      ? null
      : aug.prereqs.forEach(p => {
          let pa = augmentationData[p]
          if (!pa) return
          pa.desired = true
          propagateDesired(pa)
        })
...
async function joinFactions (ns) {
  ...
  const accessibleAugmentations = new Set(
    joinedFactions.flatMap(fac => factionData[fac]?.augmentations ?? [])
  )
  log(
    ns,
  ...
  log(
    ns,
    `Outstanding invitations from ${
      invitations.length
    } factions: ${JSON.stringify(invitations.map(f => f.name))}`
  )
  ...
  for (const faction of invitations.sort(factionSortOrder)) {
    ...
    log(
      ...
      `${faction.name} has ${faction.augmentations.length} augs, ${unownedAugs.length} unowned, ${newAugs.length} not offered by joined factions, ` +
        ...
        (desiredAugs.length == 0
          ? ' (not joining)'
          : `: ${JSON.stringify(desiredAugs)}`)
    )
    if (
      desiredAugs.length == 0 &&
      !options['force-join'].includes(faction.name)
    )
      continue
    if (
      manualJoin.includes(faction.name) &&
      !options['force-join'].includes(faction.name)
    )
      ...
      log(
        ns,
        `Joining faction ${faction.name} which has ${desiredAugs.length} desired augmentations: ${desiredAugs}`
      )
      let response
      if (
        (response = await getNsDataThroughFile(
          ns,
        ...
        log(
          ns,
          `Joined faction ${faction.name} (Response: ${response})`,
          true,
          'success'
        )
      } else
        log(
          ns,
          `Error joining faction ${faction.name}. Response: ${response}`,
          false,
          'error'
        )
    }
...
let getReqDonationForRep = (rep, faction) =>
  Math.ceil(
    (1e6 *
      Math.max(
        0,
        rep - (faction.name ? faction : factionData[faction]).reputation
      )) /
      playerData.faction_rep_mult
  )
let getReqDonationForAug = (aug, faction) =>
  getReqDonationForRep(aug.reputation, faction)

let getTotalCost = augPurchaseOrder =>
  augPurchaseOrder.reduce(
    (total, aug, i) => total + aug.price * augCountMult ** i,
    0
  )
...
function sortAugs (ns, augs = []) {
  ...
  for (let i = 0; i < augs.length; i++) {
    for (let j = 0; j < augs[i].prereqs.length; j++) {
      ...
      if (
        prereqIndex === -1 /* Already bought */ ||
        prereqIndex < i /* Already sorted up */
      )
        continue
  ...
  for (let i = augs.length - 1; i > 0; i--) {
    ...
    while (i - batchLengh >= 0 && augs[i].price > augs[i - batchLengh].price)
      batchLengh++
    if (batchLengh == 1) continue // Not the start of a batch of prerequisites
    log(
      ns,
      `Detected a batch of length ${batchLengh} from ${
        augs[i - batchLengh + 1].name
      } to ${augs[i].name}`
    )
    let moved = 0,
  ...
  if (totalMoves > 0)
    log(
      ns,
      `Cost reduced by ${formatMoney(
        initialCost - finalCost
      )} (from ${formatMoney(initialCost)} to ${formatMoney(
        finalCost
...
async function manageUnownedAugmentations (
  ns,
  ignoredAugs,
  alsoPrintToTerminal
) {
  const outputRows = [
    `Currently have ${ownedAugmentations.length}/30 Augmentations required for Daedalus.`
  ]
  ...
  if (unownedAugs.length == 0)
    return log(
      ns,
      `All ${
        Object.keys(augmentationData).length
      } augmentations are either owned or ignored!`,
      alsoPrintToTerminal
    )
  ...
  let availableAugs = await manageFilteredSubset(
    ns,
    outputRows,
    'Available',
    unownedAugs.filter(aug => aug.getFromJoined() != null),
  ...
  if (availableAugs?.length > 0) {
    let augsWithRep = availableAugs.filter(
      aug =>
        aug.canAfford() ||
        (aug.canAffordWithDonation() && !options['disable-donations'])
    )
    let desiredAugs = availableAugs.filter(aug => aug.desired)
    if (augsWithRep.length > desiredAugs.length) {
      augsWithRep = await manageFilteredSubset(
        ns,
        outputRows,
        'Within Rep',
        augsWithRep
      )
      desiredAugs = await manageFilteredSubset(
        ns,
        outputRows,
        'Desired',
        desiredAugs
...
async function manageFilteredSubset (
  ...
  subsetName,
  subset,
  printList = undefined /* undefined => automatically print if sort order changed */,
  purchaseable = false
) {
    ...
    for (const aug of subset.slice())
      if (
        ...
        aug.prereqs.some(
          prereq =>
            !(
              ownedAugmentations.includes(prereq) ||
              subset.some(a => a.name === prereq)
            )
        )
      )
  ...
  if (
    printList === true ||
    (printList === undefined && !subset.every((v, i) => v == subsetSorted[i]))
  )
    // If the purchase order is unchanged after filtering out augmentations, don't bother reprinting the full list
    outputRows.push(
      `${
        subset.length
  ...
  outputRows.push(
    `Total Cost of ${subset.length} ${subsetName}:`.padEnd(37) +
      ` ${formatMoney(totalRepCost + totalAugCost)}` +
      (totalRepCost == 0
        ? ''
        : ` (Augs: ${formatMoney(totalAugCost)} + Rep: ${formatMoney(
            totalRepCost
          )})  Donate: ${JSON.stringify(repCostByFaction).replaceAll(
            ',',
            ', '
          )}`)
  )
  ...
  if (
    totalAugCost + totalRepCost <= playerData.money &&
    options['neuroflux-disabled']
  )
    return subsetSorted
  ...
  while (totalAugCost + totalRepCost > playerData.money) {
    ...
    let costBefore = `${formatMoney(
      totalRepCost + totalAugCost
    )} (Augs: ${formatMoney(totalAugCost)} + Rep: ${formatMoney(totalRepCost)})`
    purchaseableAugs = sortAugs(
      ns,
      purchaseableAugs.filter(aug => aug !== mostExpensiveAug)
    )
    purchaseFactionDonations = computeAugsRepReqDonationByFaction(
      ns,
      purchaseableAugs
    )
    totalRepCost = Object.values(purchaseFactionDonations).reduce(
      (t, r) => t + r,
      0
    )
    totalAugCost = getTotalCost(purchaseableAugs)
    let costAfter = `${formatMoney(
      totalRepCost + totalAugCost
    )} (Augs: ${formatMoney(totalAugCost)} + Rep: ${formatMoney(totalRepCost)})`
  ...
  manageFilteredSubset(
    ...
    'Affordable',
    purchaseableAugs,
    purchaseableAugs.length < subset.length
  )
  // Let us know how far away we are from being able to get just one more aug:
  if (dropped.length > 0)
    outputRows.push(
      `Insufficient funds: had to drop ${dropped.length} augs. Next aug \"${dropped[0].aug.name}\" at: ${dropped[0].costBefore}`
    )
  ...
  if (!options['neuroflux-disabled']) {
    ...
    if (!augNf.canAfford() && !augNf.canAffordWithDonation()) {
      ...
      const factionsWithAugAndInvite = factionsWithAug
        .filter(f => f.invited || f.joined)
        .sort((a, b) => b.favor - a.favor)
      const factionWithMostFavor =
        factionsWithAugAndInvite[0] ?? factionsWithAug[0]
      if (
        getFrom != factionsWithAug[0].name &&
        factionsWithAug[0] != factionsWithAugAndInvite[0]
      )
        outputRows.push(
        ...
        factionsWithAug[0].joined &&
        !factionsWithAug[0].donationsUnlocked
      )
        outputRows.push(
          `SUGGESTION: Do some work for faction ${factionsWithAug[0].name} to qickly earn rep for ${strNF} since it has the most favor (${factionsWithAug[0].favor}).`
        )
      else if (
        (!getFrom || factionData[getFrom].favor < factionWithMostFavor.favor) &&
        factionWithMostFavor.invited
      ) {
        outputRows.push(
          `Attempting to join faction ${factionWithMostFavor.name} to make it easier to get rep for ${strNF} since it has the most favor (${factionWithMostFavor.favor}).`
        )
        options['force-join'].push(factionWithMostFavor.name)
        await joinFactions(ns)
        if (
          joinedFactions.includes(factionWithMostFavor.name) &&
          factionWithMostFavor.donationsUnlocked
        )
    ...
    log(
      ns,
      `nfPurchased: ${nfPurchased}, augNfFaction: ${augNfFaction.name} (rep: ${augNfFaction.reputation}), augNf.price: ${augNf.price}, augNf.reputation: ${augNf.reputation}`
    )
    while (nfPurchased < 50) {
      const nextNfCost =
        augNf.price *
        augCountMult ** purchaseableAugs.length *
        nfCountMult ** nfPurchased
      const nextNfRep = augNf.reputation * nfCountMult ** nfPurchased
      let nfMsg = `Cost of NF ${nfPurchased + 1} is ${formatMoney(
        nextNfCost
      )} and will require ${formatNumberShort(nextNfRep)} reputation`
      ...
      if (nextNfRep > augNfFaction.reputation) {
        if (augNfFaction.donationsUnlocked) {
          purchaseFactionDonations[augNfFaction.name] = Math.max(
            purchaseFactionDonations[augNfFaction.name] || 0,
          ...
          nfMsg += `, which will require a donation of ${formatMoney(
            purchaseFactionDonations[augNfFaction.name]
          )} to faction ${augNfFaction.name}`
        } else {
          outputRows.push(
            nfMsg +
              `, but we only have ${formatNumberShort(
                augNfFaction.reputation
              )} reputation with faction ${augNfFaction.name}!`
          )
          break
        }
      } else
        nfMsg += ` (✓ have ${formatNumberShort(
          augNfFaction.reputation
        )} rep with faction ${augNfFaction.name})`
      log(ns, nfMsg)
      nfPurchased++
    }
    log(
      ns,
      `Can afford to purchase ${nfPurchased} levels of ${strNF}. New total cost: ${formatMoney(
        totalRepCost + totalAugCost
      )}` +
        (totalRepCost == 0
          ? ''
    ...
    outputRows.push(
      `Total Cost of ${purchaseableAugs.length} (with +${nfPurchased} NF):`.padEnd(
        37
      ) +
        ` ${formatMoney(totalRepCost + totalAugCost)}` +
        (totalRepCost == 0
...
function computeAugsRepReqDonationByFaction (ns, augmentations) {
  ...
  for (const aug of augmentations) {
    ...
    let fDonationsIndex = Object.keys(repCostByFaction).findIndex(
      f => f == faction.name
    )
    let alternativeFaction = Object.keys(repCostByFaction).find(
      (f, i) =>
        f != faction.name &&
        factionData[f].augmentations.includes(aug.name) &&
        ((repCostByFaction[f] >= reqDonation &&
          (fDonationsIndex == -1 || i < fDonationsIndex)) || // We're donating the same or more to the other faction, and were planning on donating to it before this one
          getReqDonationForAug(aug, f) - repCostByFaction[f] <
            reqDonation - (repCostByFaction[faction.name] || 0))
    ) // The amount we've committed to donating the other faction is closer to this requirement
    if (alternativeFaction) {
      log(
        ns,
        `Using alternative faction "${alternativeFaction}" for "${aug.name}" rather than earlier faction "${faction.name}"`
      )
    ...
    if (reqDonation > 0)
      repCostByFaction[faction.name] = Math.max(
        repCostByFaction[faction.name] || 0,
        reqDonation
      )
  }
...
async function purchaseDesiredAugs (ns, verbose) {
  let totalRepCost = Object.values(purchaseFactionDonations).reduce(
    (t, r) => t + r,
    0
  )
  let totalAugCost = getTotalCost(purchaseableAugs)
  let money = (
    await getNsDataThroughFile(ns, 'ns.getPlayer()', '/Temp/player-info.txt')
  ).money
  if (totalAugCost + totalRepCost > money)
    return log(
      ns,
      `ERROR: Purchase order total cost (${formatMoney(
        totalRepCost + totalAugCost
      )}` +
        (totalRepCost == 0
          ? ''
        ...
        ` is more than current player money (${formatMoney(money)}).`,
      verbose,
      'error'
    )
  // Donate to factions if necessary (using a ram-dodging script of course)
  if (
    Object.keys(purchaseFactionDonations).length > 0 &&
    Object.values(purchaseFactionDonations).some(v => v > 0)
  ) {
    if (
      await getNsDataThroughFile(
        ns,
        JSON.stringify(
          ...
          '.reduce((success, o) => success && ns.donateToFaction(o.faction, o.repDonation), true)',
  ...
  if (purchaseableAugs.length == 0)
    log(
      ns,
      `INFO: Cannot afford to buy any augmentations at this time.`,
      verbose
    )
  else if (
    await getNsDataThroughFile(
      ns,
      JSON.stringify(
        purchaseableAugs.map(aug => ({
          faction: aug.getFromJoined(),
          augmentation: aug.name
        }))
      ) +
        '.reduce((success, o) => ns.purchaseAugmentation(o.faction, o.augmentation) && success, true)',
      '/Temp/facman-purchase-augs.txt'
    )
  )
    log(
      ns,
      `SUCCESS: Purchased ${purchaseableAugs.length} desired augmentations in optimal order!`,
      verbose,
      'success'
    )
  else
    log(
      ns,
      `ERROR: Failed to purchase one or more augmentations.`,
      verbose,
      'error'
    )
}
...
/** @param {NS} ns **/
function displayFactionSummary (
  ns,
  alsoPrintToTerminal,
  sortBy,
  unique,
  overrideFinishedFactions,
  excludedStats
) {
  let noAugs = Object.values(factionData).filter(
    f => f.unownedAugmentations().length == 0
  )
  if (noAugs.length > 0)
    log(
      ns,
      `${
        noAugs.length
      } factions have no augmentations to purchase (excluding NF): ${JSON.stringify(
        noAugs.map(a => a.name)
      )}`,
      alsoPrintToTerminal
    )
  let summaryFactions = Object.values(factionData).filter(
    f =>
      f.unownedAugmentations().length > 0 &&
      !overrideFinishedFactions.includes(f.name)
  )
  if (summaryFactions.length == 0) return
  // Apply any override faction options
  joinedFactions.push(
    ...overrideFinishedFactions.filter(f => !joinedFactions.includes(f))
  )
  ...
  const relevantAugStats = allAugStats.filter(
    s =>
      ...
      undefined !==
        summaryFactions.find(f =>
          f
            .unownedAugmentations()
            .find(aug => 1 != (augmentationData[aug].stats[s] || 1))
        )
  )
  ...
  let getFactionSummary = faction => {
    ...
    return (
      ...
      `${String(faction.unownedAugmentations().length).padStart(2)} / ${String(
        faction.augmentations.length
      ).padEnd(2)} ` +
      relevantAugStats
        .map(key =>
          (totalMults[key] === undefined
            ? '-'
            : totalMults[key].toPrecision(3)
          ).padStart(Math.max(shorten(key).length, 4))
        )
        .join(' ')
    )
  }
  ...
  let sortFunction = (a, b) => {
    let aMultiContrib = a.totalUnownedMults()[sortBy] || 1,
    ...
    if (unique && bMultiContrib > 1 && aMultiContrib > 1 && sort2 != 0)
      return sort2 // When in "unique" mode it's important to first list contributing factions we've already joined
    ...
    let sort4 =
      a.mostExpensiveAugCost().length - b.mostExpensiveAugCost().length // If tied, "soonest to unlock", estimated by their most expensive aug cost
  ...
  let getSeparator = faction =>
    moreContributors &&
    !(moreContributors = faction.totalUnownedMults()[sortBy] !== undefined)
