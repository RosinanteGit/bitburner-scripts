  ...
  while (true) {
    ...
    for (let iTarget = 0; iTarget < target.length; iTarget++) {
      ...
      if (remainingTime.get(target[iTarget]) <= 0) {
        ...
        var HPercent =
          ns.formulas.hacking.hackPercent(fserver[iTarget], player) * 100 // in %
        var GPercent =
          (ns.formulas.hacking.growPercent(fserver[iTarget], 1, player, 1) -
            1) *
          100 // in %
        ...
        for (let i = 0; i < nbusableServers; i++) {
          ...
          maxRam.push(
            ns.getServerMaxRam(usableServers[i]) -
              constantRam * (usableServers[i] == 'home') -
              ns.getServerUsedRam(usableServers[i])
          ) //getting total RAM I can use that doesnt include the OP script
          ...
          if ((ms <= cs) & !wIsRunning) {
            ...
            var maxWeakThreads =
              Math.ceil(
                (ns.getServerSecurityLevel(target[iTarget]) -
                  ns.getServerMinSecurityLevel(target[iTarget])) /
                  0.05
              ) + 1

            //We attribute the number of threads to run
            weakThreads[i] = Math.max(
              Math.min(
          ...
          if ((ma < mm) & (cs == ms) & !gIsRunning & !wIsRunning) {
            ...
            let nbCouple = Math.floor(
              remainingRam / (secdw * weakscriptRam + secdg * growscriptRam)
            )
            remainingRam -=
              nbCouple * (secdw * weakscriptRam + secdg * growscriptRam)
            ...
            var serveurPercent =
              fserver[iTarget]['moneyAvailable'] / fserver[iTarget]['moneyMax']
            var maxgrowThreads =
              Math.ceil(
                Math.log(1 / serveurPercent) / Math.log(GPercent / 100 + 1)
              ) + 1 //Getting the amount of threads I need to grow to 100% of the funds
            var maxweakThreads = Math.ceil((maxgrowThreads / secdg) * secdw) + 1

            //We attribute the number of threads to run
            weakThreads[i] = Math.max(
              Math.min(
                nbCouple * secdw + nbAddWeakThreads + nbCompleteWeakThreads,
                maxweakThreads - sumWeakThreads
              ),
              0
            )
            growThreads[i] = Math.max(
              Math.min(
                nbCouple * secdg + nbAddGrowThreads + nbCompleteGrowThreads,
          ...
          if (
            ...
            var weakRatio =
              Math.ceil((weakRatio / growRatio) * Math.ceil(growRatio)) + 1
            var growRatio = Math.ceil(growRatio) + 1
            var hackRatio = Math.floor(
              (hackRatio / growRatio) * Math.ceil(growRatio)
            )
            ...
            var hackCouple = Math.floor(
              remainingRam /
                (hackscriptRam +
                  (growRatio / hackRatio) * growscriptRam +
                  (weakRatio / hackRatio) * weakscriptRam)
            )
            var growCouple = Math.ceil((growRatio / hackRatio) * hackCouple)
            var weakCouple = Math.ceil(
              (secdw / secdg) * growCouple + (secdw / secdh) * hackCouple
            )
            remainingRam -=
              hackCouple * hackscriptRam +
              growCouple * growscriptRam +
              weakCouple * weakscriptRam
            ...
            var maxgrowThreads =
              Math.ceil(Math.log(2) / Math.log(GPercent / 100 + 1)) + 1 //Getting the amount of threads I need to grow back to 100% of the funds to be conservative
            var maxweakThreads =
              Math.ceil((maxgrowThreads / secdg) * secdw) +
              Math.ceil((maxhackThreads / secdh) * secdw) +
              1 //Getting the amount of threads I need to bring back security to minimum
        ...
        if ((ma < mm) & (cs == ms) & (sumGrowThreads != 0)) {
          ...
          waitingTime.set(
            target[iTarget],
            Math.max(WeakTime[iTarget], GrowTime[iTarget] + gOffset) + 100
          )
          for (let i = 0; i < nbusableServers; i++) {
        ...
        if ((ma == mm) & (cs == ms) & (sumHackThreads != 0)) {
          ...
          waitingTime.set(
            ...
            Math.max(
              WeakTime[iTarget],
              GrowTime[iTarget] + gOffset,
              HackTime[iTarget] + hOffset
            ) + 100
          )
          for (let i = 0; i < nbusableServers; i++) {
      ...
      if (
        (waitingTime.get(target[iTarget]) != null) &
        (startTime.get(target[iTarget]) != null)
      ) {
        remainingTime.set(
          target[iTarget],
          waitingTime.get(target[iTarget]) -
            (time - startTime.get(target[iTarget]))
        )
      }
      await ns.sleep(0)
    }
...
async function findHackable (ns, allServers, player) {
  ...
  for (let i = 0; i < allServers.length; i++) {
    ...
    if (
      ns.getHackingLevel() >= ns.getServerRequiredHackingLevel(allServers[i]) &&
      numPortsPossible >= ns.getServerNumPortsRequired(allServers[i])
    ) {
    ...
    let freeRam =
      ns.getServerMaxRam(allServers[i]) - ns.getServerUsedRam(allServers[i])
    if (
      (ns.hasRootAccess(allServers[i]) ||
        numPortsPossible >= ns.getServerNumPortsRequired(allServers[i])) &
      (freeRam > 0)
    ) {
