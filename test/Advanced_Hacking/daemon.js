...
export async function main (ns) {
  ...
  while (doLoop) {
    ...
    // if we aren't at max money, fix that
      ...
      let cyclesAvailable = Math.floor(
        (freeRam - costForGrowScheduler) / singleCycleCost
      )
      let threadsForGrow = Math.floor(cyclesAvailable * 5)
      let threadsForWeaken = Math.floor(cyclesAvailable * 4)
    ...
    // if we're at min sec and we've done initial growth, calculate and perform HWGW cycles.
      ...
      let threadsForHack = Math.ceil(
        ns.hackAnalyzeThreads(target, amountToSteal)
      )
      ...
      let threadsForGrow = Math.ceil(
        ns.growthAnalyze(target, coForGrowth, ns.getServer(hostName).cpuCores) *
          1.05
      )
      ...
      let threadsToWeakenFromHack = Math.ceil(
        secIncreaseFromHack / threadPotencyForWeaken
      )
      let threadsToWeakenFromGrow = Math.ceil(
        secIncreaseFromGrow / threadPotencyForWeaken
      )
      let totalWeakenCost =
        (threadsToWeakenFromGrow + threadsToWeakenFromHack) * costForWeaken

      // calculate how many threads we can run at once
      let ramAvailable =
        ns.getServerMaxRam(hostName) - ns.getServerUsedRam(hostName)
      let totalCycleCost =
        totalHackCost + totalGrowCost + totalWeakenCost + costForSchedulers
      let cycleThreadsAvailable = Math.floor(ramAvailable / totalCycleCost)

      // calculate total amount of time for a single cycle
      let totalCycleTime = timeForWeaken + windowDelay
      ...
      if (cycleThreadsAvailable > 1 && percentageToSteal < 99) {
        //max of 99%
        ns.print(
          'Based on ' +
            totalCycleCost +
            ' cycle cost, percentage to steal of ' +
            percentageToSteal +
            '% is too low. Adjusting for next run-loop.'
        )
        percentageToSteal += 1
        skipHackDueToCycleImperfection = true
      } else if (cycleThreadsAvailable < 1 && percentageToSteal > 1) {
        //minimum of 1%
        ns.print(
          'Current percentage to steal of ' +
            percentageToSteal +
            '% is too high for even 1 thread. Adjusting for next run-loop.'
        )
        percentageToSteal -= 1
        skipHackDueToCycleImperfection = true
      }
      ...
      if (!skipHackDueToCycleImperfection) {
        ...
        let argsForHack = [
          target,
          threadsToWeakenFromHack,
          threadsForHack,
          timeForWeaken,
          timeForHack
        ]
        // run hack-scheduler first using hack args
        ns.run(
          'hack-scheduler.js',
          1,
          argsForHack[0],
        ...
        // run grow-scheduler approx. 6s later to repair after our hack
        ns.run(
          'grow-scheduler.js',
          1,
          argsForGrow[0],
        ...
        let delay = totalCycleTime + activationDelay + delayForKill
        ns.print('Sleeping for ' + ns.tFormat(delay, true))
        await ns.sleep(delay)
      }
    }
