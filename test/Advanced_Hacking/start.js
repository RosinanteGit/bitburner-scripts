...
export async function main (ns) {
  ...
  while (doLoop) {
    ...
    if (
      lastTarget == null ||
      newTarget.hostname != lastTarget.hostname ||
      !ns.scriptRunning('daemon.js', host)
    ) {
      ...
      if (lastTarget != null) {
        let lastArgs = [
          lastTarget.hostname,
          lastTarget.moneyMax,
          lastTarget.minDifficulty
        ]
        if (
          ns.isRunning('daemon.js', host, lastArgs[0], lastArgs[1], lastArgs[2])
        ) {
      ...
      if (lastTarget == null || newTarget.hostname != lastTarget.hostname) {
        ns.tprint(
          'New target ' + newArgs[0] + ' discovered; creating new daemon...'
        )
      }
      // run a new daemon
...
function sortServers (ns) {
  ...
  for (let server of servers) {
    ...
    if (ns.fileExists('Formulas.exe', 'home')) {
      ...
      let money =
        ns.formulas.hacking.hackChance(obj, player) *
        ns.formulas.hacking.hackPercent(obj, player) *
        obj.moneyMax
  ...
  while (stack.length > 0) {
    ...
    if (!serversFound.has(server)) {
      ...
      for (let serv of neighbors) {
        if (!serversFound.has(serv)) stack.push(serv)
      }
    }
  }
