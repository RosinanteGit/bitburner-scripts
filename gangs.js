...
let options
const argsSchema = [
  ['training-percentage', 0.2], // Spend this percent of time training gang members versus doing crime
...
export async function main (ns) {
  ...
  if (sf2Level == 0)
    return log(
      ns,
      'ERROR: You have no yet unlocked gangs. Script should not be run...'
    )
  ...
  while (true) {
    try {
      ...
      log(
        ns,
        `ERROR: Caught an unhandled error in the main loop: ${String(err)}`,
        'error',
        true
      )
    }
...
async function initialize (ns) {
  ...
  while (
    !(await getNsDataThroughFile(
      ns,
      'ns.gang.inGang()',
      '/Temp/player-gang-joined.txt'
    ))
  ) {
    if (!loggedWaiting) {
      log(
        ns,
        `Waiting to be in a gang. Will create the highest faction gang as soon as it is available...`
      )
      loggedWaiting = true
    }
    await runCommand(
      ns,
      `${JSON.stringify(gangsByPower)}.forEach(g => ns.gang.createGang(g))`,
  ...
  if (loggedWaiting)
    log(ns, `SUCCESS: Created gang ${myGangFaction}`, 'success', true)
  ...
  if (sf4Level == 0)
    ...
    try {
      ...
      const augmentationNames = await getNsDataThroughFile(
        ns,
        `ns.getAugmentationsFromFaction('${myGangFaction}')`,
        '/Temp/gang-augs.txt'
      )
      ...
      requiredRep = augmentationNames
        .filter(
          aug => !ownedAugmentations.includes(aug) && aug != 'The Red Pill'
        )
        .reduce((max, aug) => Math.max(max, dictAugRepReqs[aug]), -1)
      log(
        ns,
        `Highest augmentation reputation cost is ${formatNumberShort(
          requiredRep
        )}`
      )
    } catch {
  ...
  if (requiredRep == -1) requiredRep = 2.5e6

  // Initialize equipment information
  const equipmentNames = await getNsDataThroughFile(
    ns,
    'ns.gang.getEquipmentNames()',
    '/Temp/gang-equipment-names.txt'
  )
  const dictEquipmentTypes = await getGangInfoDict(
    ns,
    equipmentNames,
    'getEquipmentType'
  )
  ...
  const dictEquipmentStats = await getGangInfoDict(
    ns,
    equipmentNames,
    'getEquipmentStats'
  )
  ...
  allTaskNames = await getNsDataThroughFile(
    ns,
    'ns.gang.getTaskNames()',
    '/Temp/gang-task-names.txt'
  )
  ...
  myGangMembers = await getNsDataThroughFile(
    ns,
    'ns.gang.getMemberNames()',
    '/Temp/gang-member-names.txt'
  )
  const dictMembers = await getGangInfoDict(
    ns,
    myGangMembers,
  ...
  for (const member of Object.values(dictMembers)) // Initialize the current activity of each member
    assignedTasks[member.name] =
      member.task && member.task !== 'Unassigned'
        ? member.task
        : 'Train ' + (isHackGang ? 'Hacking' : 'Combat')
...
async function mainLoop (ns) {
  ...
  if (
    ...
    territoryTickDetected &&
    Date.now() + updateInterval >= territoryNextTick
  ) {
    isReadyForNextTerritoryTick = true
    await updateMemberActivities(ns, null, 'Territory Warfare')
  } else if (!territoryTickDetected) {
    // Detect the first territory tick by watching for other gang's territory power to update.
    const otherGangInfo = await getNsDataThroughFile(
      ns,
      'ns.gang.getOtherGangInformation()',
      '/Temp/gang-other-gang-info.txt'
    ) // Returns dict of { [gangName]: { "power": Number, "territory": Number } }
    if (
      lastOtherGangInfo != null &&
      Object.keys(otherGangInfo).some(
        g => otherGangInfo[g].power != lastOtherGangInfo[g].power
      )
    ) {
  ...
  if (
    ...
    Date.now() > territoryNextTick + 5 * updateInterval
  ) {
...
async function onTerritoryTick (ns, myGangInfo) {
  ...
  if (lastTerritoryPower != myGangInfo.power)
    log(
      ns,
      `Territory power updated from ${formatNumberShort(
        lastTerritoryPower
      )} to ${formatNumberShort(myGangInfo.power)}.`
    )
  if (!isReadyForNextTerritoryTick)
    log(ns, `WARNING: Territory tick happend before we were ready!`, 'warning')
  if (!warfareFinished)
    // Once we hit 100% territory, there's no need to keep swapping members to warfare
  ...
  const nextMemberCost = Math.pow(
    5,
    myGangMembers.length - (3 /*numFreeMembers*/ - 1)
  )
  if (
    myGangMembers.length < 12 /* Game Max */ &&
    myGangInfo.respect * 0.75 > nextMemberCost
  )
    // Don't spend more than 75% of our respect on new members.
  ...
  const task =
    Math.random() >= pctTraining
      ...
      : 'Train ' +
        (Math.random() < 0.1
          ? 'Charisma'
          : Math.random() < (isHackGang ? 0.1 : 0.9)
          ? 'Combat'
          : 'Hacking')
  await updateMemberActivities(ns, dictMembers, task) // Set everyone working on the next activity
  if (!task)
    await optimizeGangCrime(ns, await waitForGameUpdate(ns, myGangInfo)) // Finally, see if we can improve rep gain rates by micro-optimizing individual member crimes
}
...
async function updateMemberActivities (
  ...
  const dictMembers =
    dictMemberInfo ||
    (await getGangInfoDict(ns, myGangMembers, 'getMemberInformation'))
  const workOrders = []
  ...
  if (
    await getNsDataThroughFile(
      ...
      `${JSON.stringify(
        workOrders
      )}.reduce((success, m) => success && ns.gang.setMemberTask(m.name, m.task), true)`,
      '/Temp/gang-set-member-tasks.txt'
    )
  )
    log(
      ns,
      `INFO: Assigned ${workOrders.length} gang member tasks! (${workOrders
        .map(o => o.task)
    ...
    log(
      ns,
      `ERROR: Failed to set member task of one or more members: ` +
...
async function optimizeGangCrime (ns, myGangInfo) {
  ...
  const playerData = await getNsDataThroughFile(
    ns,
    'ns.getPlayer()',
  ...
  if (ownedSourceFiles[4] > 0) {
    try {
      factionRep = await getNsDataThroughFile(
        ns,
        `ns.getFactionRep('${myGangFaction}')`,
        `/Temp/gang-faction-rep.txt`
      )
    } catch {
      log(
        ns,
        'INFO: Error suppressed. Falling back to estimating current gang faction rep.'
      )
    }
  }
  if (factionRep == -1)
  ...
  const optStat =
    factionRep > requiredRep
      ...
      ? 'respect'
      : 'both money and respect' // Change priority based on achieved rep/money
  ...
  for (let shuffle = 0; shuffle < 1000; shuffle++) {
    ...
    shuffleArray(myGangMembers.slice()).forEach((member, index) => {
      ...
      const bestTask =
        ...
        Date.now() - (lastMemberReset[member] || 0) <
          options['min-training-ticks'] * territoryTickTime
          ? taskRates.find(
              t => t.name === 'Train ' + (isHackGang ? 'Hacking' : 'Combat')
            )
          : totalWanted > wantedGainTolerance || sustainableTasks.length == 0
          ? taskRates.find(t => t.name === 'Vigilante Justice')
          : sustainableTasks[0]
    ...
    while (
      ...
      Object.values(proposedTasks).some(t => t.name !== 'Vigilante Justice')
    ) {
      const mostWanted = Object.keys(proposedTasks).reduce(
        (t, c) =>
          proposedTasks[c].name !== 'Vigilante Justice' &&
      ...
      const nextBestTask =
        memberTaskRates[mostWanted].filter(
          c => c.wanted < proposedTasks[mostWanted].wanted
        )[0] ??
        memberTaskRates[mostWanted].find(t => t.name === 'Vigilante Justice')
    ...
    if (
      ...
      [bestTaskAssignments, bestTotalGain, bestWanted] = [
        proposedTasks,
        totalGain,
        totalWanted
      ]
  ...
  if (
    bestTaskAssignments != null &&
    myGangMembers.some(m => assignedTasks[m] !== bestTaskAssignments[m].name)
  ) {
    ...
    const [optWanted, optRespect, optMoney] = myGangMembers
      ...
      .reduce(([w, r, m], t) => [w + t.wanted, r + t.respect, m + t.money], [
        0,
        0,
        0
      ])
    ...
    log(
      ns,
      `SUCCESS: Optimized gang member crimes for ${optStat} with wanted gain tolerance ${wantedGainTolerance.toPrecision(
        2
      )} (${elapsed} ms). ` +
        `Wanted: ${oldGangInfo.wantedLevelGainRate.toPrecision(
          3
        )} -> ${myGangInfo.wantedLevelGainRate.toPrecision(3)}, ` +
        `Rep: ${formatNumberShort(
          oldGangInfo.respectGainRate
        )} -> ${formatNumberShort(
          myGangInfo.respectGainRate
        )}, Money: ${formatMoney(oldGangInfo.moneyGainRate)} -> ${formatMoney(
          myGangInfo.moneyGainRate
        )}`
    )
    ...
    if (
      ...
      log(
        ns,
        `WARNING: Calculated new rates would be Rep:${formatNumberShort(
          optRespect
        )} Wanted: ${optWanted.toPrecision(3)} Money: ${formatMoney(
          optMoney
        )}` +
          `but they are Rep:${formatNumberShort(
            myGangInfo.respectGainRate
          )} Wanted: ${myGangInfo.wantedLevelGainRate.toPrecision(
            3
          )} Money: ${formatMoney(myGangInfo.moneyGainRate)}`,
        'warning'
      )
  } else
    log(
      ns,
      `INFO: Determined all gang member assignments are already optimal for ${optStat} with wanted gain tolerance ${wantedGainTolerance.toPrecision(
        2
      )} (${elapsed} ms).`
    )
...
async function fixWantedGainRate (ns, myGangInfo, wantedGainTolerance = 0) {
  ...
  log(
    ns,
    `WARNING: Generating wanted levels (${lastWantedLevelGainRate.toPrecision(
      3
    )}/sec > ${wantedGainTolerance.toPrecision(
      3
    )}/sec), temporarily assigning random members to Vigilante Justice...`,
    'warning'
  )
  for (const member of shuffleArray(myGangMembers.slice())) {
    ...
    const wantedLevelGainRate = (myGangInfo = await waitForGameUpdate(
      ns,
      myGangInfo
    )).wantedLevelGainRate
...
async function doRecruitMember (ns) {
  ...
  do {
    ...
    myGangMembers.includes(newMemberName) ||
    myGangMembers.includes(newMemberName + ' Understudy')
  )
  if (i < myGangMembers.length) newMemberName += ' Understudy' // Pay our respects to the deceased
  if (
    await getNsDataThroughFile(
      ns,
      `ns.gang.canRecruitMember() && ns.gang.recruitMember('${newMemberName}')`,
      '/Temp/gang-recruit-member.txt'
    )
  ) {
    myGangMembers.push(newMemberName)
    assignedTasks[newMemberName] =
      'Train ' + (isHackGang ? 'Hacking' : 'Combat')
    lastMemberReset[newMemberName] = Date.now()
    log(
      ns,
      `SUCCESS: Recruited a new gang member "${newMemberName}"!`,
      'success'
    )
  } else {
    log(
      ns,
      `ERROR: Failed to recruit a new gang member "${newMemberName}"!`,
      'error'
    )
  }
}
...
async function tryAscendMembers (ns) {
  const dictAscensionResults = await getGangInfoDict(
    ns,
    myGangMembers,
    'getAscensionResult'
  )
  for (const member of myGangMembers) {
    ...
    const ascMultiThreshold =
      options['ascend-multi-threshold'] /* 2.3 */ -
      Number(member.split(' ')[1]) * 0.1 /* 1.1 for member 12 */
    if (
      !ascResult ||
      !importantStats.some(stat => ascResult[stat] >= ascMultiThreshold)
    )
      continue
    if (
      undefined !==
      (await getNsDataThroughFile(
        ...
        '/Temp/gang-ascend-member.txt'
      ))
    ) {
      log(
        ns,
        `SUCCESS: Ascended member ${member} to increase multis by ${importantStats
          .map(s => `${s} -> ${ascResult[s].toFixed(2)}x`)
          .join(', ')}`,
        'success'
      )
      ...
      log(
        ns,
        `ERROR: Attempt to ascended member ${member} failed. Go investigate!`,
        'error'
      )
  }
}
...
async function tryUpgradeMembers (ns, dictMembers) {
  ...
  const homeMoney =
    playerData.money - (Number.parseFloat(ns.read('reserve.txt')) || 0)
  ...
  for (const equip of equipments) {
    for (const member of Object.values(dictMembers)) {
      ...
      let percievedCost =
        equip.cost *
        (Object.keys(equip.stats).some(stat =>
          importantStats.some(i => stat.includes(i))
        )
          ? 1
          : 50)
      ...
      if (
        !member.upgrades.includes(equip.name) &&
        !member.augmentations.includes(equip.name)
      ) {
        purchaseOrder.push({
          member: member.name,
          type: equip.type,
...
async function doUpgradePurchases (ns, purchaseOrder) {
  ...
  const orderSummary = purchaseOrder
    .map(o => `${o.member} ${o.type}: "${o.equipmentName}"`)
    .join(', ')
  if (
    await getNsDataThroughFile(
      ns,
      `${JSON.stringify(
        purchaseOrder
      )}.reduce((success, o) => success && ns.gang.purchaseEquipment(o.member, o.equipmentName), true)`,
    ...
    log(
      ...
      } gang member upgrades for ${formatMoney(
        purchaseOrder.reduce((t, e) => t + e.cost, 0)
      )}. (${orderSummary})`,
      'success'
    )
  else
    log(
      ns,
      `ERROR: Failed to purchase one or more gang member upgrades. (${orderSummary})`,
      'error'
    )
}
...
async function enableOrDisableWarfare (ns, myGangInfo) {
  warfareFinished =
    Math.round(myGangInfo.territory * 2 ** 20) /
      2 ** 20 /* Handle API imprecision */ >=
    1
  if (warfareFinished && !myGangInfo.territoryWarfareEngaged) return // No need to engage once we hit 100%
  const otherGangs = await getNsDataThroughFile(
    ns,
    'ns.gang.getOtherGangInformation()',
    '/Temp/gang-other-gang-info.txt'
  ) // Returns dict of { [gangName]: { "power": Number, "territory": Number } }
  ...
  for (const otherGang in otherGangs) {
    if (otherGangs[otherGang].territory == 0 || otherGang == myGangFaction)
      continue // *New* Don't have to worry about battling a gang if it has 0 territory
    const winChance =
      myGangInfo.power / (myGangInfo.power + otherGangs[otherGang].power)
  ...
  const shouldEngage =
    !warfareFinished && territoryEngageThreshold <= averageWinChance
  if (shouldEngage != myGangInfo.territoryWarfareEngaged) {
    log(
      ns,
      (warfareFinished ? 'SUCCESS' : 'INFO') +
        `: Toggling participation in territory warfare to ${shouldEngage}. Our power: ${formatNumberShort(
          myGangInfo.power
        )}. ` +
        (!warfareFinished
          ? `Lowest win chance is ${(100 * lowestWinChance).toFixed(
              2
            )}% with ${lowestWinChanceGang} (power ${formatNumberShort(
              otherGangs[lowestWinChanceGang]?.power
            )}). ` +
            `Average win chance ${(100 * averageWinChance).toFixed(
              2
            )}% across ${totalActiveGangs} active gangs.`
    ...
    await runCommand(
      ...
      '/Temp/gang-set-warfare.js'
    )
  }
}

// Ram-dodging helper to get gang information for each item in a list
const getGangInfoDict = async (ns, elements, gangFunction) =>
  await getDict(
    ns,
    elements,
    `gang.${gangFunction}`,
    `/Temp/gang-${gangFunction}.txt`
  )
const getDict = async (ns, elements, nsFunction, fileName) =>
  await getNsDataThroughFile(
    ns,
    `Object.fromEntries(${JSON.stringify(
      elements
    )}.map(e => [e, ns.${nsFunction}(e)]))`,
    fileName
  )
...
let getWantedPenalty = myGangInfo =>
  myGangInfo.respect / (myGangInfo.respect + myGangInfo.wantedLevel)
let getTerritoryPenalty = myGangInfo =>
  (0.2 * myGangInfo.territory + 0.8) * multGangSoftcap
...
function computeRepGains (myGangInfo, currentTask, memberInfo) {
  ...
  const territoryMult = Math.max(
    0.005,
    Math.pow(myGangInfo.territory * 100, task.territory.respect) / 100
  )
...
function computeWantedGains (myGangInfo, currentTask, memberInfo) {
  ...
  const territoryMult = Math.max(
    0.005,
    Math.pow(myGangInfo.territory * 100, task.territory.wanted) / 100
  )
  if (isNaN(territoryMult) || territoryMult <= 0) return 0
...
function calculateMoneyGains (myGangInfo, currentTask, memberInfo) {
  ...
  const territoryMult = Math.max(
    0.005,
    Math.pow(myGangInfo.territory * 100, task.territory.money) / 100
  )
  if (isNaN(territoryMult) || territoryMult <= 0) return 0
  const respectMult = getWantedPenalty(myGangInfo)
  const territoryPenalty = getTerritoryPenalty(myGangInfo)
...
function log (ns, message, toastStyle, terminal = undefined) {
  ns.print(message)
  if (terminal === true || (terminal === undefined && toastStyle === 'error'))
    ns.tprint(message)
