...
export async function main (ns) {
  ...
  const pre4sBuyThresholdProbability =
    options['pre-4s-buy-threshold-probability']
  ...
  nearTermForecastWindowLength =
    options['pre-4s-inversion-detection-window'] || 10
  longTermForecastWindowLength =
    options['pre-4s-forecast-window'] || marketCycleLength + 1
  showMarketSummary =
    options['show-pre-4s-forecast'] || options['show-market-summary']
  ...
  if (!ns.getPlayer().hasTixApiAccess)
    ...
    return log(
      ns,
      'ERROR: You have to buy stock market access and API access before you can run this script!',
      true
    )

  if (options.l || options.liquidate)
    // If given the "liquidate" command, try to kill the version of ourself trading in stocks
    await runCommand(
      ns,
  ...
  if (!disableShorts && (!(8 in dictSourceFiles) || dictSourceFiles[8] < 2)) {
    log(
      ns,
      'INFO: Shorting stocks has been disabled (you have not yet unlocked access to shorting)'
    )
    disableShorts = true
  }

  allStockSymbols = await getNsDataThroughFile(
    ns,
    'ns.stock.getSymbols()',
  ...
  if (options.l || options.liquidate) {
    ...
    // If we're not liquidating or in mock mode, we MUST not run two stockmasters at once, or chaos will ensue
    let otherStockmasters = (
      await getNsDataThroughFile(ns, `ns.ps()`, '/Temp/process-list.txt')
    ).filter(p => p.filename == ns.getScriptName()) // TODO: For bonus points, check all servers
    otherStockmasters = otherStockmasters.filter(
      p => JSON.stringify(ns.args) != JSON.stringify(p.args)
    ) // Don't detect ourselves of course.
    if (otherStockmasters.some(p => !p.args.includes('--mock')))
      // Exception, feel free to run multiple stockmasters in mock mode
      return log(
        ns,
        `ERROR: Another version of ${ns.getScriptName()} is already running with different args. Running twice is a bad idea!`,
        true,
        'error'
      )
  }
  ...
  while (true) {
    ...
    const thresholdToBuy = pre4s
      ? options['pre-4s-buy-threshold-return']
      : options['buy-threshold']
    const thresholdToSell = pre4s
      ? options['pre-4s-sell-threshold-return']
      : options['sell-threshold']
    ...
    if (pre4s && allStocks[0].priceHistory.length < minTickHistory) {
      log(
        ns,
        `Building a history of stock prices (${allStocks[0].priceHistory.length}/${minTickHistory})...`
      )
          ...
          if (!stk.warnedBadPurchase)
            log(
              ...
              `WARNING: Thinking of selling ${stk.sym} with ER ${formatBP(
                stk.absReturn()
              )}, but holding out as it was purchased just ${
                stk.ticksHeld
              } ticks ago...`
            )
    ...
    if (liquidity > fracB) {
      ...
      for (const stk of allStocks.sort(purchaseOrder)) {
        ...
        if (
          pre4s &&
          Math.max(pre4sMinHoldTime, pre4sMinBlackoutWindow) >=
            marketCycleLength - estTick
        )
          continue
        ...
        if (
          pre4s &&
          (stk.lastInversion < minTickHistory ||
        ...
        budget = Math.min(
          budget,
          (1 - fracH) * corpus * diversification - stk.positionValue()
        )
        ...
        let ticksBeforeCycleEnd =
          marketCycleLength - estTick - stk.timeToCoverTheSpread()
        ...
        let estEndOfCycleValue =
          numShares *
          purchasePrice *
          ((stk.absReturn() + 1) ** ticksBeforeCycleEnd - 1) // Expected difference in purchase price and value at next market cycle end
        if (estEndOfCycleValue <= 2 * commission)
          log(
            ns,
            `Despite attractive ER of ${formatBP(stk.absReturn())}, ${
              stk.sym
            } was not bought. Budget: ${formatMoney(
              budget
            )} can only buy ${numShares} shares @ ${formatMoney(
              purchasePrice
            )}. ` +
              `Given an estimated ${marketCycleLength -
                estTick} ticks left in market cycle, less ${stk
                .timeToCoverTheSpread()
                .toFixed(1)} ticks to cover the spread (${(
                stk.spread_pct * 100
              ).toFixed(2)}%), ` +
              `remaining ${ticksBeforeCycleEnd.toFixed(
                1
              )} ticks would only generate ${formatMoney(
                estEndOfCycleValue
              )}, which is less than 2x commission (${formatMoney(
                2 * commission,
                3
              )})`
          )
...
let getStockInfoDict = async (ns, stockFuncion) =>
  await getNsDataThroughFile(
    ns,
    `Object.fromEntries(${JSON.stringify(
      allStockSymbols
    )}.map(sym => [sym, ns.stock.${stockFuncion}(sym)]))`,
    `/Temp/stock-${stockFuncion}.txt`
  )
...
async function initAllStocks (ns, allStockSymbols) {
  ...
  return allStockSymbols.map(s => ({
    ...
    timeToCoverTheSpread: function () {
      return (
        Math.log(this.ask_price / this.bid_price) /
        Math.log(1 + this.absReturn())
      )
    },
...
async function refresh (ns, playerStats, allStocks, myStocks) {
  ...
  const dictVolatilities = !has4s
    ? null
    : await getStockInfoDict(ns, 'getVolatility')
  const dictForecasts = !has4s
    ? null
    : await getStockInfoDict(ns, 'getForecast')
  ...
  if (ticked) {
    if (Date.now() - lastTick < expectedTickTime - sleepInterval) {
      if (Date.now() - lastTick < catchUpTickTime - sleepInterval) {
        let changedPrices = allStocks.filter(
          stk => stk.ask_price != dictAskPrices[stk.sym]
        )
        log(
          ns,
          `WARNING: Detected a stock market tick after only ${formatDuration(
            ...
            (changedPrices.length >= 33
              ...
              : `The following ${
                ...
                } stock prices changed: ${changedPrices
                  .map(
                    stk =>
                      `${stk.sym} ${formatMoney(
                        stk.ask_price
                      )} -> ${formatMoney(dictAskPrices[stk.sym])}`
                  )
                  .join(', ')}`),
          false,
          'warning'
...
const forecast = history =>
  history.reduce(
    (ups, price, idx) =>
      idx == 0 ? 0 : history[idx - 1] > price ? ups + 1 : ups,
    0
  ) /
  (history.length - 1)
...
const detectInversion = (p1, p2) =>
  (p1 >= 0.5 + tol2 &&
    p2 <= 0.5 - tol2 &&
    p2 <= 1 - p1 + inversionDetectionTolerance) ||
  /* Reverse Condition: */ (p1 <= 0.5 - tol2 &&
    p2 >= 0.5 + tol2 &&
    p2 >= 1 - p1 - inversionDetectionTolerance)
...
async function updateForecast (ns, allStocks, has4s) {
  ...
  const prepSummary =
    ...
    mock ||
    (!has4s &&
      (currentHistory < minTickHistory ||
        allStocks.filter(stk => stk.owned()).length == 0)) // Decide whether to display the market summary table.
  ...
  for (const stk of allStocks) {
    ...
    if (!has4s)
      stk.vol = stk.priceHistory.reduce(
        (max, price, idx) =>
          Math.max(
            max,
            idx == 0 ? 0 : Math.abs(stk.priceHistory[idx - 1] - price) / price
          ),
        0
      )
    ...
    stk.nearTermForecast = forecast(
      stk.priceHistory.slice(0, nearTermForecastWindowLength)
    )
  ...
  if (inversionsDetected.length > 0) {
    summary += `${
      inversionsDetected.length
    } Stocks appear to be reversing their outlook: ${inversionsDetected
      .map(s => s.sym)
      .join(', ')} (threshold: ${inversionAgreementThreshold})\n`
    if (
      inversionsDetected.length >= inversionAgreementThreshold &&
  ...
  for (const stk of allStocks) {
    ...
    if (
      ...
      ((has4s && detectedCycleTick == 0) ||
        (!has4s &&
          detectedCycleTick > nearTermForecastWindowLength / 2 - 1 &&
          detectedCycleTick <=
            nearTermForecastWindowLength + inversionLagTolerance))
    )
    ...
    const probWindowLength = Math.min(
      longTermForecastWindowLength,
      stk.lastInversion
    )
    ...
    if (prepSummary) {
      ...
      stk.debugLog =
        ...
        `Prob:${(stk.prob * 100)
          .toFixed(0)
          .padStart(3)}% (t${probWindowLength.toFixed(0).padStart(2)}:${(
          stk.longTermForecast * 100
        )
          .toFixed(0)
          .padStart(3)}%, ` +
        `t${Math.min(stk.priceHistory.length, nearTermForecastWindowLength)
          .toFixed(0)
          .padStart(2)}:${(stk.nearTermForecast * 100)
          .toFixed(0)
          .padStart(3)}%) ` +
        `tLast⇄:${(stk.lastInversion + 1).toFixed(0).padStart(3)} Vol:${(
          stk.vol * 100
        ).toFixed(2)}% ER:${formatBP(stk.expectedReturn()).padStart(8)} ` +
        `Spread:${(stk.spread_pct * 100).toFixed(2)}% ttProfit:${stk
          .blackoutWindow()
          .toFixed(0)
          .padStart(3)}`
      if (stk.owned())
        stk.debugLog += ` Pos: ${formatNumberShort(stk.ownedShares(), 3, 1)} (${
          stk.ownedShares() == stk.maxShares
            ? 'max'
            : ((100 * stk.ownedShares()) / stk.maxShares)
                .toFixed(0)
                .padStart(2) + '%'
        }) ${stk.sharesLong > 0 ? 'long ' : 'short'} (held ${
          stk.ticksHeld
        } ticks)`
  ...
  if (prepSummary) {
    summary +=
      `Market day ${detectedCycleTick + 1}${
        marketCycleDetected ? '' : '?'
      } of ${marketCycleLength} (${
        marketCycleDetected
          ? ((100 * inversionAgreementThreshold) / 19).toPrecision(2)
          : '0'
      }% certain) ` +
...
let updateForecastFile = async (ns, summary) =>
...
let buyStockWrapper = async (ns, sym, numShares) =>
  await transactStock(ns, sym, numShares, 'buy') // ns.stock.buy(sym, numShares);
let buyShortWrapper = async (ns, sym, numShares) =>
  await transactStock(ns, sym, numShares, 'short') // ns.stock.short(sym, numShares);
let sellStockWrapper = async (ns, sym, numShares) =>
  await transactStock(ns, sym, numShares, 'sell') // ns.stock.sell(sym, numShares);
let sellShortWrapper = async (ns, sym, numShares) =>
  await transactStock(ns, sym, numShares, 'sellShort') // ns.stock.sellShort(sym, numShares);
let transactStock = async (ns, sym, numShares, action) =>
  await getNsDataThroughFile(
    ns,
    `ns.stock.${action}('${sym}', ${numShares})`,
    '/Temp/transact-stock.txt'
  ) // ns.stock.sellShort(sym, numShares);
...
async function doBuy (ns, stk, sharesBought) {
  ...
  try {
    price = mock
      ? expectedPrice
      : Number(
          await transactStock(ns, stk.sym, sharesBought, long ? 'buy' : 'short')
        )
  } catch (err) {
    if (long) throw err
    disableShorts = true
  ...
  log(
    ns,
    `INFO: ${long ? 'Bought ' : 'Shorted'} ${formatNumberShort(
      sharesBought,
      3,
      3
    ).padStart(5)}${
      stk.maxShares == sharesBought + stk.ownedShares() ? ' (max shares)' : ''
    } ` +
      `${stk.sym.padEnd(5)} @ ${formatMoney(price).padStart(
        9
      )} for ${formatMoney(sharesBought * price).padStart(9)} (Spread:${(
        stk.spread_pct * 100
      ).toFixed(2)}% ` +
      `ER:${formatBP(stk.expectedReturn()).padStart(
        8
      )}) Ticks to Profit: ${stk.timeToCoverTheSpread().toFixed(2)}`,
    noisy,
    'info'
  )
  ...
  if (price == 0) {
    ...
    log(
      ns,
      `WARNING: ${long ? 'Bought' : 'Shorted'} ${stk.sym} @ ${formatMoney(
        price
      )} but expected ${formatMoney(expectedPrice)} (spread: ${formatMoney(
        stk.spread
      )})`,
      false,
      'warning'
  ...
  if (mock && long)
    stk.boughtPrice =
      (stk.boughtPrice * stk.sharesLong + price * sharesBought) /
      (stk.sharesLong + sharesBought)
  if (mock && !long)
    stk.boughtPriceShort =
      (stk.boughtPriceShort * stk.sharesShort + price * sharesBought) /
      (stk.sharesShort + sharesBought)
  if (long) stk.sharesLong += sharesBought
  else stk.sharesShort += sharesBought // Maintained for mock mode, otherwise, redundant (overwritten at next refresh)
...
async function doSellAll (ns, stk) {
  ...
  let price = mock
    ? expectedPrice
    : await transactStock(ns, stk.sym, sharesSold, long ? 'sell' : 'sellShort')
  const profit =
    (long
      ? stk.sharesLong * (price - stk.boughtPrice)
      : stk.sharesShort * (stk.boughtPriceShort - price)) -
    2 * commission
  log(
    ns,
    `${profit > 0 ? 'SUCCESS' : 'WARNING'}: Sold all ${formatNumberShort(
      sharesSold,
      3,
      3
    ).padStart(5)} ${stk.sym.padEnd(5)} ${
      long ? ' long' : 'short'
    } positions ` +
      `@ ${formatMoney(price).padStart(9)} for a ` +
      (profit > 0
        ? `PROFIT of ${formatMoney(profit).padStart(9)}`
        : ` LOSS  of ${formatMoney(-profit).padStart(9)}`) +
      ` after ${stk.ticksHeld} ticks`,
    noisy,
    noisy ? (profit > 0 ? 'success' : 'error') : undefined
  )
  if (price == 0) {
    log(
      ns,
      `ERROR: Failed to sell ${sharesSold} ${stk.sym} ${
        long ? 'shares' : 'shorts'
      } @ ${formatMoney(expectedPrice)} - 0 was returned.`,
      true,
      'error'
    )
    return 0
    ...
    log(
      ns,
      `WARNING: Sold ${stk.sym} ${long ? 'shares' : 'shorts'} @ ${formatMoney(
        price
      )} but expected ${formatMoney(expectedPrice)} (spread: ${formatMoney(
        stk.spread
      )})`,
...
function doStatusUpdate (ns, stocks, myStocks, hudElement = null) {
  ...
  let est_holdings_cost = myStocks.reduce(
    (sum, stk) =>
      sum +
      (stk.owned() ? commission : 0) +
      stk.sharesLong * stk.boughtPrice +
      stk.sharesShort * stk.boughtPriceShort,
    0
  )
  let liquidation_value = myStocks.reduce(
  ...
  let status =
    ...
    (myStocks.length == 0
      ? ''
      : `(ER ${minReturnBP.toFixed(1)}-${maxReturnBP.toFixed(1)} BP) `) +
    `Profit: ${formatMoney(totalProfit, 3)} Holdings: ${formatMoney(
      liquidation_value,
      3
    )} (Cost: ${formatMoney(est_holdings_cost, 3)}) ` +
    `Net: ${formatMoney(
      totalProfit + liquidation_value - est_holdings_cost,
      3
    )}`
  log(ns, status)
...
async function liquidate (ns, allStockSymbols) {
  ...
  for (const sym of allStockSymbols) {
    ...
    if (sharesLong > 0)
      totalRevenue +=
        (await sellStockWrapper(ns, sym, sharesLong)) * sharesLong - commission
    if (sharesShort > 0)
      totalRevenue +=
        (2 * avgShortCost - (await sellShortWrapper(ns, sym, sharesShort))) *
          sharesShort -
        commission
  }
  log(
    ns,
    `Sold ${totalSharesLong.toLocaleString()} long shares and ${totalSharesShort.toLocaleString()} short shares ` +
      `in ${totalStocks} stocks for ${formatMoney(totalRevenue, 3)}`,
    true,
    'success'
  )
}
...
async function tryGet4SApi (
  ...
  if (totalCost > corpus * 0.9 /* Need to reserve some money to invest */)
    return false
  if (playerStats.money < totalCost) await liquidate(ns, allStockSymbols)
  if (!playerStats.has4SData) {
    if (
      await getNsDataThroughFile(
        ns,
        'ns.stock.purchase4SMarketData()',
        '/Temp/purchase-4s.txt'
      )
    )
      log(
        ns,
        `Purchased 4SMarketData for ${formatMoney(cost4sData)}!`,
        true,
        'success'
      )
  ...
  if (
    await getNsDataThroughFile(
      ...
      '/Temp/purchase-4s-api.txt'
    )
  ) {
    log(
      ns,
      `Purchased 4SMarketDataTixApi for ${formatMoney(cost4sApi)}!`,
      true,
      'success'
    )
    return true
...
function initializeHud () {
  ...
  let customElements = d.getElementById('overview-extra-hook-0').parentElement
    .parentElement
  ...
  stockValueTracker
    .querySelectorAll('p')
    .forEach((el, i) => (el.id = 'stock-display-' + i))
  ...
  customElements.parentElement.insertBefore(
    stockValueTracker,
    customElements.parentElement.childNodes[2]
  )
  return htmlDisplay
}
