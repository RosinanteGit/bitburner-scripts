...
export async function main (ns) {
  ...
  var ram_total = servers.reduce(function (total, server) {
    if (
      ...
      (server.hostname.startsWith('hacknet') && !flags['include-hacknet-ram'])
    )
      return total
  ...
  function getRatesAtHackLevel (server, player, hackLevel) {
    ...
    try {
      const weakenCost =
        weaken_ram * ns.formulas.hacking.weakenTime(server, player)
      const growCost =
        grow_ram * ns.formulas.hacking.growTime(server, player) +
        (weakenCost * 0.004) / 0.05
      const hackCost =
        hack_ram * ns.formulas.hacking.hackTime(server, player) +
      ...
      server.estHackPercent = Math.min(
        0.98,
        Math.min(
          (ram_total * hackGain) / hackCost,
          1 - 1 / Math.exp((ram_total * growGain) / growCost)
        )
      ) // TODO: I think these might be off by a factor of 2x
      ...
      const theoreticalGainRate =
        (hackProfit /
          (growCost * grows_per_cycle + hackCost * hacks_per_cycle)) *
      ...
      const cappedGainRate = Math.min(
        theoreticalGainRate,
        hackProfit / ram_total
      )
      ns.print(
        `At hack level ${hackLevel} and steal ${(
          hack_percent * 100
        ).toPrecision(3)}%: Theoretical ${formatMoney(theoreticalGainRate)}, ` +
          `Limit: ${formatMoney(
            hackProfit / ram_total
          )}, Exp: ${expRate.toPrecision(3)}, Hack Chance: ${(
  ...
  ns.print(
    `All? ${flags['all']} Player hack: ${player.hacking} Ram total: ${ram_total}`
  )
  ...
  const unlocked_servers = servers
    .filter(s => s.requiredHackingSkill <= player.hacking)
    .map(function (server) {
      ;[
        server.theoreticalGainRate,
        server.gainRate,
        server.expRate
      ] = getRatesAtHackLevel(server, player, player.hacking)
      return server
    })
  ...
  const locked_servers =
    servers
      ...
      .map(function (server) {
        ...
        const [
          bestUnlockedScaledGainRate,
          _,
          bestUnlockedScaledExpRate
        ] = getRatesAtHackLevel(
          best_unlocked_server,
          player,
          server.requiredHackingSkill
        )
        const gainRateScaleFactor =
          best_unlocked_server.theoreticalGainRate / bestUnlockedScaledGainRate
        const expRateScaleFactor =
          best_unlocked_server.expRate / bestUnlockedScaledExpRate
        const [
          theoreticalGainRate,
          cappedGainRate,
          expRate
  ...
  if (!flags['silent'])
    ns.tprint(
      ...
      ' with ',
      formatMoney(best_server.gainRate),
      ' per ram-second'
    )
  let order = 1
  ...
  if (!flags['silent'])
    ns.tprint(
      'Best exp server: ',
      best_exp_server.hostname,
      ' with ',
      best_exp_server.expRate,
      ' exp per ram-second'
    )
  order = 1
  ...
  for (let i = 0; i < 5; i++)
    serverListByExp += `\n ${order++} ${
      server_eval[i].hostname
    }, with ${server_eval[i].expRate.toPrecision(3)} exp per ram-second`
