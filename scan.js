...
export let main = ns => {
  ...
  let s = ['home'],
    ...
    fName = x => {
      ...
      return (
        `<span class="w" id="${x}">` +
        `<a class="s${factionServers.includes(x) ? ' f' : ''}${
          rooted ? ' r' : ''
        }">${x}</a>
                <span class="hack ${
  ...
  let ordering = (a, b) => {
    ...
    d =
      d != 0
        ? d
        : serverInfo(b).purchasedByPlayer - serverInfo(a).purchasedByPlayer // Purchased servers to the very top
    d =
      d != 0
        ? d
        : a
            .slice(0, 2)
            .toLowerCase()
            .localeCompare(b.slice(0, 2).toLowerCase()) // Hack: compare just the first 2 chars to keep purchased servers in order purchased
    return d
  }
  ...
  doc
    .querySelectorAll('.sc.new .backdoor')
    .forEach(q =>
      q.addEventListener(
        'click',
        tcommand.bind(
          null,
          r[q.parentNode.childNodes[0].childNodes[0].nodeValue] + ';backdoor'
        )
      )
    )
