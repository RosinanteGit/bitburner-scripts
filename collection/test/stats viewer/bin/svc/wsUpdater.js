 ...
 * @module
 */
const CONFIG_PATH = '/etc/svc/wsUpdater/config.txt'
/**
 ...
 */
async function readFile (ns, filename) {
 ...
 */
async function writeFile (ns, filename, data, allowRestart = false) {
  let normFilename = normalizeFilename(filename)
...
export async function main (ns) {
  ...
  let configString = await readFile(ns, CONFIG_PATH)
  ...
  try {
    ...
    ns.print(
      `Couldn't read configuration file at ${CONFIG_PATH}, using default configuration.`
    )
  }
  ...
  ws.onclose = () => {
    ns.toast(
      `Closed connection to update server at '${config.serverUrl}'.`,
      'warning'
    )
    ns.exit()
  }
  ...
  ws.onerror = () => {
    ns.toast(
      `Error connecting to update server at '${config.serverUrl}'.`,
      'error'
    )
  }
