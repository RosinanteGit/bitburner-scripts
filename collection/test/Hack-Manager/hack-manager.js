/* Finds the optimal server to hack and hacks it from all possible servers except home. Only run from home server */
/** @param {import("../..").NS } ns */
  ...
  while (true) {
    ...
    if (ns.getServerSecurityLevel(target) > securityThresh) {
      ...
      for (let i = 0; i < rootableServers.length; i++) {
        ...
        numThreads =
          ns.getServerMaxRam(rootableServers[i]) -
          ns.getServerUsedRam(rootableServers[i]) //free ram
      ...
      for (let i = 0; i < rootableServers.length; i++) {
        ...
        numThreads =
          ns.getServerMaxRam(rootableServers[i]) -
          ns.getServerUsedRam(rootableServers[i])
...
async function findAllServers (ns) {
  const fileList = ['h1.ns', 'w1.ns', 'g1.ns'] //These files just infinitely hack, weaken, and grow respectively.
  var q = []
...
async function findOptimal (ns, hackableServers) {
  ...
  for (let i = 0; i < hackableServers.length; i++) {
    ...
    currTime =
      ns.getWeakenTime(hackableServers[i]) +
      ns.getGrowTime(hackableServers[i]) +
      ns.getHackTime(hackableServers[i])
    currVal /= currTime
