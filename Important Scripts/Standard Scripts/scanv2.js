...
export async function main (ns) {
  ...
  servers.forEach(server => {
    ...
    if (
      !(ns.args[0] != undefined && server.depth > ns.args[0]) &&
      server.depth != 0
    ) {
      ...
      output +=
        baseDashes +
        `> <a style="cursor:pointer; color:${color}; text-decoration: underline;" onclick='
...
let svObj = (name = 'home', depth = 0, path = '') => ({
  name: name,
  depth: depth,
  path: path
})
