...
export async function main (ns) {
  ...
  ns.tail()
  if (!crime || ns.args.includes('--fast-crimes-only'))
    // More sophisticated auto-scaling crime logic
    await crimeForKillsKarmaStats(
      ns,
      0,
      0,
      Number.MAX_SAFE_INTEGER,
      ns.commitCrime,
      ns.args.includes('--fast-crimes-only')
    )
  // Simple crime loop for the specified crime
  else await legacyAutoCrime(ns, crime)
}
...
async function legacyAutoCrime (ns, crime = 'mug') {
  ...
  while (true) {
    ...
    ns.print(
      `Karma: ${formatNumberShort(
        ns.heart.break()
      )} Committing crime \"${crime}\" and sleeping for ${formatDuration(
        wait
      )}...`
    )
    await ns.sleep(wait)
