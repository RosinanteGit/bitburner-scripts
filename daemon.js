...
const lowUtilizationThreshold = 0.8 // The counterpart - low utilization, which leads us to ramp up targets
...
function doesFileExist (filename, hostname = undefined) {
...
function ps (ns, server, canUseCache = true) {
  ...
  return canUseCache && cachedResult
    ? cachedResult
    : (psCache[server] = ns.ps(server))
}
...
function shouldReserveMoney () {
  ...
  if (!doesFileExist('SQLInject.exe', 'home')) {
    if (playerMoney > 20000000) return true // Start saving at 200m of the 250m required for SQLInject
  } else if (!playerStats.has4SDataTixApi) {
    if (
      playerMoney >=
      (bitnodeMults.FourSigmaMarketDataApiCost * 25000000000) / 2
    )
...
export async function main (ns) {
  ...
  log(
    'The following source files are active: ' + JSON.stringify(dictSourceFiles)
  )
  //ns.disableLog('ALL');
  disableLogs(ns, [
    'getServerMaxRam',
  ...
  stockMode =
    options.s ||
    options['stock-manipulation'] ||
    options['stock-manipulation-focus']
  ...
  if (stockFocus)
    log(
      '--stock-manipulation-focus - Stock market manipulation is the main priority'
    )
  ...
  asynchronousHelpers = [
    {
      name: 'stats.js',
      shouldRun: () =>
        ns.getServerMaxRam('home') >= 64 /* Don't waste precious RAM */
    }, // Adds stats not usually in the HUD
    {
      name: 'hacknet-upgrade-manager.js',
      args: ['-c', '--max-payoff-time', '1h']
    }, // Kickstart hash income by buying everything with up to 1h payoff time immediately
    {
      name: 'stockmaster.js',
      args: ['--show-market-summary'],
      tail: true,
      shouldRun: () => playerStats.hasTixApiAccess
    }, // Start our stockmaster if we have the required stockmarket access
    { name: 'gangs.js', tail: true, shouldRun: () => 2 in dictSourceFiles }, // Script to create manage our gang for us
    {
      ...
      args: ['-v'],
      shouldRun: () => 9 in dictSourceFiles
    ...
    { name: 'sleeve.js', tail: true, shouldRun: () => 10 in dictSourceFiles }, // Script to create manage our sleeves for us
    {
      ...
      args: ['--fast-crimes-only', '--no-coding-contracts'], // Singularity script to manage how we use our "focus" work.
      shouldRun: () =>
  ...
  let shouldUpgradeHacknet = () =>
    !shouldReserveMoney() &&
    whichServerIsRunning(ns, 'hacknet-upgrade-manager.js', false) === null
  periodicScripts = [
    // Buy tor as soon as we can if we haven't already, and all the port crackers
    {
      ...
      shouldRun: () =>
        4 in dictSourceFiles && !addedServerNames.includes('darkweb')
    },
    {
      interval: 30000,
    ...
    },
    {
      ...
      args: () => [
        ...
        '8h',
        '--max-spend',
        ns.getServerMoneyAvailable('home') * 0.01
      ]
    },
    // Don't start auto-joining factions until we're holding 1 billion (so coding contracts returning money is probably less critical) or we've joined one already
    {
      ...
      shouldRun: () =>
        4 in dictSourceFiles &&
        (playerStats.factions.length > 0 ||
          ns.getServerMoneyAvailable('home') > 1e9) &&
  ...
  if (stockFocus) maxTargets = Object.keys(serverStockSymbols).length // Ensure we immediately attempt to target all servers that represent stocks
  if (stockMode && !playerStats.hasTixApiAccess)
    log(
      "WARNING: Ran with '--stock-manipulation' flag, but this will have no effect until you buy access to the stock market API then restart or manually run stockmaster.js"
    )
...
function whichServerIsRunning (ns, scriptName, canUseCache = true) {
  for (const server of serverListByFreeRam)
    if (
      ps(ns, server.name, canUseCache).some(
        process => process.filename === scriptName
      )
    )
      return server.name
  return null
...
async function runPeriodicScripts (ns) {
  ...
  for (const task of periodicScripts) {
    ...
    if (
      Date.now() - (task.lastRun || 0) >= task.interval &&
      (task.shouldRun === undefined || task.shouldRun())
    ) {
  ...
  if (
    9 in dictSourceFiles &&
    ns.getServerMoneyAvailable('home') < 10000000 &&
    ns.getServerMaxRam('home') - ns.getServerUsedRam('home') >= 5.6
  ) {
    await runCommand(
      ns,
      `0; if(ns.hacknet.spendHashes("Sell for Money")) ns.toast('Sold 4 hashes for \$1M', 'success')`,
      '/Temp/sell-hashes-for-money.js'
    )
  }
}
...
async function tryRunTool (ns, tool) {
  if (!doesFileExist(tool.name)) {
    log(
      `ERROR: Tool ${tool.name} was not found on ${daemonHost}`,
      true,
      'error'
    )
    return false
  ...
  if (runningOnServer != null) {
    if (verbose)
      log(
        `INFO: Tool ${tool.name} is already running on server ${runningOnServer}.`
      )
    return true
  }
  const args = tool.args
    ? tool.args instanceof Function
      ? tool.args()
      : tool.args
    : [] // Support either a static args array, or a function returning the args.
  const runResult = await arbitraryExecution(
    ns,
    tool,
    1,
    args,
    tool.requiredServer || 'home'
  ) // TODO: Allow actually requiring a server
  if (runResult) {
    runningOnServer = whichServerIsRunning(ns, tool.name, false)
    if (verbose)
      log(
        `Ran tool: ${tool.name} on server ${runningOnServer}` +
          (args.length > 0 ? ` with args ${JSON.stringify(args)}` : '')
      )
    if (tool.tail === true) {
      log(
        `Tailing Tool: ${tool.name} on server ${runningOnServer}` +
          (args.length > 0 ? ` with args ${JSON.stringify(args)}` : '')
      )
    ...
    log(
      `WARNING: Tool cannot be run (insufficient RAM? REQ: ${formatRam(
        tool.cost
      )}): ${tool.name}`,
      false,
      'warning'
...
async function doTargetingLoop (ns) {
  ...
  do {
    ...
    try {
      ...
      if (loops % 60 == 0) {
        ...
        if (verbose && loops % 600 == 0)
          log(
            'Targetting Order:\n  ' +
              serverListByTargetOrder
                .filter(s => s.shouldHack())
                .map(
                  s =>
                    `${s.isPrepped() ? '*' : ' '} ${
                      s.canHack() ? '✓' : 'X'
                    } Money: ${formatMoney(s.getMoney(), 4)} of ${formatMoney(
                      s.getMaxMoney(),
                      4
                    )} ` +
                    `(${formatMoney(
                      s.getMoneyPerRamSecond(),
                      4
                    )}/ram.sec), Sec: ${formatNumber(
                      s.getSecurity(),
                      3
                    )} of ${formatNumber(s.getMinSecurity(), 3)}, ` +
                    `TTW: ${formatDuration(s.timeToWeaken())}, Hack: ${
                      s.requiredHackLevel
                    } - ${s.name}` +
                    (!stockMode || !serverStockSymbols[s.name]
                      ? ''
                      : ` Sym: ${
                          serverStockSymbols[s.name]
                        } Owned: ${serversWithOwnedStock.includes(s.name)} ` +
                        `Manip: ${
                          shouldManipulateGrow[s.name]
                            ? 'grow'
                            : shouldManipulateHack[s.name]
                            ? 'hack'
                            : '(disabled)'
                        }`)
                )
      ...
      var firstUnpreppedServerIndex = serverListByTargetOrder.findIndex(
        s => s.shouldHack() && s.canHack() && !s.isPrepped() && !s.isTargeting()
      )
      if (firstUnpreppedServerIndex !== -1 && !stockMode)
        serverListByTargetOrder.unshift(
          serverListByTargetOrder.splice(firstUnpreppedServerIndex, 1)[0]
        )
      ...
      let isWorkCapped = () =>
        (workCapped =
          workCapped ||
          failed.length > 0 || // Scheduling fails when there's insufficient RAM. We've likely encountered a "soft cap" on ram utilization e.g. due to fragmentation
          ...
          targeting.length + prepping.length >=
            maxTargets + maxPreppingAtMaxTargets) // Only allow a couple servers to be prepped in advance when at max-targets
      ...
      for (var i = 0; i < serverListByTargetOrder.length; i++) {
        if (Date.now() - start >= maxLoopTime) {
          // To avoid lagging the game, completely break out of the loop if we start to run over
        ...
        if (!server.hasRoot() && server.canCrack()) doRoot(server)

        // Check whether we can / should attempt any actions on this server
        if (!server.shouldHack()) {
          // Ignore servers we own (bought servers / home / no money)
          noMoney.push(server)
        } else if (!server.hasRoot()) {
          // Can't do anything to servers we have not yet cracked
          notRooted.push(server)
        } else if (!server.canHack()) {
          // Note servers above our Hack skill. We can prep them a little if we have spare RAM at the end.
          cantHack.push(server)
          lowestUnhackable = Math.min(
            lowestUnhackable,
            server.requiredHackLevel
          )
          // New logic allows for unhackable servers to be prepping. Keep tabs on how many we have of each
          if (server.isPrepped()) cantHackButPrepped.push(server)
          else if (server.isPrepping()) cantHackButPrepping.push(server)
        } else if (server.isTargeting()) {
          // Note servers already being targeted from a prior loop
          targeting.push(server) // TODO: While targeting, we should keep queuing more batches
        } else if (server.isPrepping()) {
          // Note servers already being prepped from a prior loop
          prepping.push(server)
        } else if (isWorkCapped() || xpOnly) {
          // Various conditions for which we'll postpone any additional work on servers (computed at the end of each loop)
          if (
            xpOnly &&
            ((nextXpCycleEnd[server.name] || 0) > start - 10000 ||
              server.isXpFarming())
          )
            targeting.push(server)
          // A server counts as "targeting" if in XP mode and its due to be farmed or was in the past 10 seconds
          else skipped.push(server)
        } else if (!hackOnly && true == (await prepServer(ns, server))) {
          // Returns true if prepping, false if prepping failed, null if prepped
          if (server.previouslyPrepped)
            log(
              `WARNING ${server.prepRegressions++}: Server was prepped, but now at security: ${formatNumber(
                server.getSecurity()
              )} ` +
                `(min ${formatNumber(
                  server.getMinSecurity()
                )}) money: ${formatMoney(
                  server.getMoney(),
                  3
                )} (max ${formatMoney(server.getMaxMoney(), 3)}). ` +
                `Prior cycle: ${
                  server.previousCycle
                }. ETA now (Hack ${playerHackSkill()}) is ${formatDuration(
                  server.timeToWeaken()
                )}`,
              true,
              'warning'
        ...
        } else if (!hackOnly && !server.isPrepped()) {
          // If prepServer returned false or null. Check ourselves whether it is prepped
          log(
            'Prep failed for "' +
              server.name +
              '" (RAM Utilization: ' +
              (getTotalNetworkUtilization() * 100).toFixed(2) +
              '%)'
          )
          failed.push(server)
        } else if (targeting.length >= maxTargets) {
          // Hard cap on number of targets, changes with utilization
          server.previouslyPrepped = true
          preppedButNotTargeting.push(server)
        } else {
          // Otherwise, server is prepped at min security & max money and ready to target
          ...
          if (server.actualPercentageToSteal() === 0) {
            // Not enough RAM for even one hack thread of this next-best target.
            failed.push(server)
          } else if (
            true == (await performScheduling(ns, server, performanceSnapshot))
          ) {
            // once conditions are optimal, fire barrage after barrage of cycles in a schedule
            targeting.push(server)
          } else {
            log(
              'Targeting failed for "' +
                server.name +
                '" (RAM Utilization: ' +
                (getTotalNetworkUtilization() * 100).toFixed(2) +
                '%)'
    ...
    do {
      ...
      if (!isWorkCapped() && cantHack.length > 0 && !hackOnly && !xpOnly) {
        ...
        cantHack.sort(function (a, b) {
          ...
          return diff != 0.0
            ? diff
            : b.getMoneyPerRamSecond() - a.getMoneyPerRamSecond() // Break ties by sorting by max-money
        })
        // Try to prep them all unless one of our capping rules are hit
        // TODO: Something is not working right here, so until we figure it out, never look at more than the first unhackable server.
        for (var j = 0; j < 1 /*cantHack.length*/; j++) {
          ...
          if (isWorkCapped()) break
          if (
            cantHackButPrepped.includes(server) ||
            cantHackButPrepping.includes(server)
          )
            continue
          var prepResult = await prepServer(ns, server)
          if (prepResult == true) {
            ...
            log(
              'Pre-Prep failed for "' +
                ...
                '" with ' +
                server.requiredHackLevel +
                ' hack requirement (RAM Utilization: ' +
                (getTotalNetworkUtilization() * 100).toFixed(2) +
                '%)'
            )
            failed.push(server)
      ...
      let network = getNetworkStats()
      let utilizationPercent = network.totalUsedRam / network.totalMaxRam
      highUtilizationIterations =
        utilizationPercent >= maxUtilization ? highUtilizationIterations + 1 : 0
      lowUtilizationIterations =
        utilizationPercent <= lowUtilizationThreshold
          ? lowUtilizationIterations + 1
          : 0

      // If we've been at low utilization for longer than the cycle of all our targets, we can add a target
      let intervalsPerTargetCycle =
        targeting.length == 0
          ? 120
          : Math.ceil(
              (targeting.reduce(
                (max, t) => Math.max(max, t.timeToWeaken()),
                0
              ) +
                cycleTimingDelay) /
                loopInterval
      ...
      //log(`intervalsPerTargetCycle: ${intervalsPerTargetCycle} lowUtilizationIterations: ${lowUtilizationIterations} loopInterval: ${loopInterval}`);
      if (
        (lowUtilizationIterations > intervalsPerTargetCycle ||
          utilizationPercent < 0.01) &&
        skipped.length > 0 &&
        maxTargets < serverListByTargetOrder.length
      ) {
        maxTargets++
        log(
          `Increased max targets to ${maxTargets} since utilization (${formatNumber(
            utilizationPercent * 100,
            3
          )}%) has been quite low for ${lowUtilizationIterations} iterations.`
        )
        lowUtilizationIterations = 0 // Reset the counter of low-utilization iterations
      } else if (highUtilizationIterations > 60) {
        // Decrease max-targets by 1 ram utilization is too high (prevents scheduling efficient cycles)
        maxTargets -= 1
        log(
          `Decreased max targets to ${maxTargets} since utilization has been > ${formatNumber(
            maxUtilization * 100,
            3
          )}% for 60 iterations and scheduling failed.`
        )
        highUtilizationIterations = 0 // Reset the counter of high-utilization iterations
      }
      maxTargets = Math.max(maxTargets, targeting.length - 1, 1) // Ensure that after a restart, maxTargets start off with no less than 1 fewer max targets
      ...
      if (xpOnly) {
        // If all we want to do is gain hack XP
        let time = await kickstartHackXp(ns, 1.0, verbose)
        loopInterval = Math.min(1000, time || 1000) // Wake up earlier if we're almost done an XP cycle
      } else if (!workCapped && lowUtilizationIterations > 10) {
        let expectedRunTime = getXPFarmServer().timeToHack()
        let freeRamToUse =
          expectedRunTime < loopInterval // If expected runtime is fast, use as much RAM as we want, it'll all be free by our next loop.
            ? 1 - (1 - lowUtilizationThreshold) / (1 - utilizationPercent) // Take us just up to the threshold for 'lowUtilization' so we don't cause unecessary server purchases
            : 1 -
              (1 - maxUtilizationPreppingAboveHackLevel - 0.05) /
                (1 - utilizationPercent) // Otherwise, leave more room (e.g. for scheduling new batches.)
        await kickstartHackXp(
          ns,
          freeRamToUse,
          verbose &&
            (expectedRunTime > 10000 || lowUtilizationIterations % 10 == 0),
          1
        )
      }
      ...
      if (notRooted.length > 0)
        keyUpdates += `, ${
          notRooted.length
        } are not rooted (missing ${portCrackers
          .filter(c => !c.exists())
          .map(c => c.name)
          .join(',')})`
      if (cantHack.length > 0)
      ...
      keyUpdates += `\n > Targeting: ${
        targeting.length
      } servers, Prepping: ${prepping.length + cantHackButPrepping.length}`
      if (xpOnly)
        keyUpdates += `\n > Grinding XP from ${targeting
          .map(s => s.name)
          .join(', ')}`
      // To reduce log spam, only log if some key status changes, or if it's been a minute
      if (keyUpdates != lastUpdate || Date.now() - lastUpdateTime > 60000) {
        log(
          (lastUpdate = keyUpdates) +
            '\n > RAM Utilization: ' +
            formatRam(Math.ceil(network.totalUsedRam)) +
            ' of ' +
            formatRam(network.totalMaxRam) +
            ' (' +
            (utilizationPercent * 100).toFixed(1) +
            '%) ' +
            `for ${lowUtilizationIterations ||
              highUtilizationIterations} its, Max Targets: ${maxTargets}, Loop Took: ${Date.now() -
              start}ms`
        )
        lastUpdateTime = Date.now()
      }
      ...
      log(
        'WARNING: Caught an error in the targeting loop: ' + err,
        true,
        'warning'
      )
      // Note netscript errors are raised as a simple string (no message property)
      ...
      // Catch errors that appear to be caused by deleted servers, and remove the server from our lists.
      const expectedDeletedHostPhrase = 'Invalid IP/hostname: '
      let expectedErrorPhraseIndex = errorMessage.indexOf(
        expectedDeletedHostPhrase
      )
      if (expectedErrorPhraseIndex == -1) continue
      let start = expectedErrorPhraseIndex + expectedDeletedHostPhrase.length
      let lineBreak = errorMessage.indexOf('<br>', start)
      ...
      log(
        'INFO: The server "' +
          deletedHostName +
          '" appears to have been deleted. Removing it from our lists',
        false,
        'info'
      )
      removeServerByName(deletedHostName)
...
let actualWeakenPotency = () =>
  bitnodeMults.ServerWeakenRate *
  weakenThreadPotency *
  (1 - weakenThreadPadding)

// Dictionaries of static server information
let serversDictCommand = (servers, command) =>
...
async function refreshDynamicServerData (ns, serverNames) {
  ...
  dictServerMaxMoney = await getNsDataThroughFile(
    ns,
    serversDictCommand(serverNames, 'ns.getServerMaxMoney(server)'),
    '/Temp/servers-max-money.txt'
  )
  // Get the information about the relative profitability of each server
  const pid = ns.exec(
    getFilePath('analyze-hack.js'),
    'home',
    1,
    '--all',
    '--silent'
  ...
  if (!dictServerProfitInfo)
    return log(ns, 'WARN: analyze-hack info unavailable.')
  dictServerProfitInfo = Object.fromEntries(
    JSON.parse(dictServerProfitInfo).map(s => [s.hostname, s])
  )
  //ns.print(dictServerProfitInfo);
  if (options.i)
    currentTerminalServer = getServerByName(
      await getNsDataThroughFile(ns, 'ns.getCurrentServer()')
    )
}
...
function buildServerObject (ns, node) {
  return {
    ...
    getMoneyPerRamSecond: () =>
      dictServerProfitInfo
        ? dictServerProfitInfo[node]?.gainRate ?? 0
        : dictServerMaxMoney[node] ?? 0,
    getExpPerSecond: () =>
      dictServerProfitInfo
        ? dictServerProfitInfo[node]?.expRate ?? 0
        : 1 / dictServerMinSecurityLevels[node] ?? 0,
    ...
    shouldHack: function () {
      return (
        this.getMaxMoney() > 0 &&
        this.name !== 'home' &&
        !this.name.startsWith('hacknet-node-') &&
        !this.name.startsWith(purchasedServersName)
      ) // Hack, but beats wasting 2.25 GB on ns.getPurchasedServers()
    },
    ...
    isPrepped: function () {
      ...
      let isPrepped =
        (currentSecurity == 0 ||
          this.getMinSecurity() / currentSecurity >= 0.99) &&
        ((this.getMaxMoney() != 0 &&
          currentMoney / this.getMaxMoney() >= 0.99) ||
          stockFocus) /* Only prep security in stock-focus mode */
      return isPrepped
    },
    // Function to tell if the sever is running any tools, with optional filtering criteria on the tool being run
    isSubjectOfRunningScript: function (
      filter,
      useCache = true,
      count = false
    ) {
      ...
      for (const hostname of addedServerNames)
        for (const process of ps(ns, hostname, useCache))
          if (
            toolNames.includes(process.filename) &&
            process.args[0] == this.name &&
    ...
    isPrepping: function (useCache = true) {
      return this.isSubjectOfRunningScript(
        process => process.args.length > 4 && process.args[4] == 'prep',
        useCache
      )
    },
    isTargeting: function (useCache = true) {
      return this.isSubjectOfRunningScript(
        process => process.args.length > 4 && process.args[4].includes('Batch'),
        useCache
      )
    },
    isXpFarming: function (useCache = true) {
      return this.isSubjectOfRunningScript(
        process =>
          process.args.length > 4 && process.args[4].includes('FarmXP'),
        useCache
      )
    },
    serverGrowthPercentage: function () {
      return (
        (this.ns.getServerGrowth(this.name) *
          bitnodeMults.ServerGrowthRate *
          getPlayerHackingGrowMulti()) /
        100
      )
    },
    adjustedGrowthRate: function () {
      return Math.min(
        maxGrowthRate,
        1 + (unadjustedGrowthRate - 1) / this.getMinSecurity()
      )
    },
    ...
    targetGrowthCoefficientAfterTheft: function () {
      return (
        1 /
        (1 - this.getHackThreadsNeeded() * this.percentageStolenPerHackThread())
      )
    },
    ...
    percentageStolenPerHackThread: function () {
      ...
      return Math.min(
        ...
        Math.max(
          0,
          (((100 - Math.min(100, this.getMinSecurity())) / 100) *
            ((playerHackSkill() - (this.requiredHackLevel - 1)) /
              playerHackSkill())) /
            240
        )
      )
    ...
    getGrowThreadsNeeded: function () {
      return Math.min(
        ...
        Math.ceil(
          (
            this.cyclesNeededForGrowthCoefficient() /
    ...
    getWeakenThreadsNeededAfterTheft: function () {
      return Math.ceil(
        (
          (this.getHackThreadsNeeded() * hackThreadHardening) /
          actualWeakenPotency()
        ).toPrecision(14)
      )
    },
    getWeakenThreadsNeededAfterGrowth: function () {
      return Math.ceil(
        (
          (this.getGrowThreadsNeededAfterTheft() * growthThreadHardening) /
          actualWeakenPotency()
        ).toPrecision(14)
      )
    },
    ...
    hasRoot: function () {
      return (
        this._hasRootCached ||
        (this._hasRootCached = this.ns.hasRootAccess(this.name))
      )
    },
    isHost: function () {
    ...
    weakenThreadsNeeded: function () {
      return Math.ceil(
        (
          (this.getSecurity() - this.getMinSecurity()) /
          actualWeakenPotency()
        ).toPrecision(14)
      )
    }
...
function getNetworkStats () {
  ...
  const listOfServersFreeRam = rootedServers
    .map(s => s.ramAvailable())
    .filter(ram => ram > 1.6) // Servers that can't run a script don't count
  const totalMaxRam = rootedServers
    .map(s => s.totalRam())
    .reduce((a, b) => a + b, 0)
...
function getPerformanceSnapshot (currentTarget, networkStats) {
  ...
  const weaken1Cost =
    currentTarget.getWeakenThreadsNeededAfterTheft() * getTool('weak').cost
  const weaken2Cost =
    currentTarget.getWeakenThreadsNeededAfterGrowth() * getTool('weak').cost
  const growCost =
    currentTarget.getGrowThreadsNeededAfterTheft() * getTool('grow').cost
  ...
  const jobs = [weaken1Cost, weaken2Cost, growCost, hackCost].sort(
    (a, b) => b - a
  ) // Sort jobs largest to smallest
  ...
  while (canScheduleAnother && maxScheduled++ <= maxBatches) {
    for (const job of jobs) {
      ...
      const freeSlot = simulatedRemainingRam
        .sort((a, b) => b - a)
        .findIndex(ram => ram >= job)
      if (freeSlot === -1) canScheduleAnother = false
      else simulatedRemainingRam[freeSlot] -= job
    }
  }
  return {
    ...
    optimalPacedCycles: Math.min(
      ...
      Math.max(
        1,
        Math.floor(
          (currentTarget.timeToWeaken() / cycleTimingDelay).toPrecision(14)
        ) - 1
      )
    ), // Fudge factor, this isnt an exact scuence
...
let getTargetSummary = currentTarget =>
  ...
  (stockMode && shouldManipulateGrow[currentTarget.name]
    ? 'with grow stock '
    : stockMode && shouldManipulateHack[currentTarget.name]
    ? 'with hack stock '
    : '') +
  `to steal ${formatNumber(currentTarget.actualPercentageToSteal() * 100)}% ` +
  `(${formatMoney(
    currentTarget.actualPercentageToSteal() * currentTarget.getMaxMoney(),
    3,
    1
  )}) ` +
  `ETA: ${formatDuration(
    currentTarget.timeToWeaken()
  )} at Hack ${playerHackSkill()} (${currentTarget.name})`
...
function optimizePerformanceMetrics (currentTarget) {
  ...
  currentTarget.percentageToSteal = Math.max(
    currentTarget.percentageToSteal,
    percentPerHackThread
  ) // If the initial % to steal is below the minimum, raise it
  ...
  while (++attempts < maxAdjustments) {
    ...
    const adjustment = analyzeSnapshot(
      performanceSnapshot,
      currentTarget,
      networkStats,
      increment
    ...
    if (runOnce && verbose)
      log(
        `Adjustment ${attempts} (increment ${increment}): ${adjustment} to ${newHackThreads} hack threads ` +
          `(from ${formatNumber(
            currentTarget.actualPercentageToSteal() * 100
          )}% or ${currentTarget.getHackThreadsNeeded()} hack threads)`
      )
    if (adjustment === 0.0 && increment == 1) break // We've zeroed in on the exact number of hack threads we want
    if (adjustment === 0.0 || Math.sign(adjustment) != lastAdjustmentSign) {
      // Each time we change the direction of adjustments, slow the adjustment rate
      increment = Math.max(1, Math.floor((increment / 2.0).toPrecision(14)))
      lastAdjustmentSign =
        adjustment === 0.0 ? lastAdjustmentSign : Math.sign(adjustment)
    }
    ...
    currentTarget.percentageToSteal = Math.max(
      0,
      newHackThreads * percentPerHackThread
    )
  }
  if (
    attempts >= maxAdjustments ||
    (verbose &&
      currentTarget.actualPercentageToSteal() != oldActualPercentageToSteal)
  ) {
    log(
      `Tuned % to steal from ${formatNumber(
        oldActualPercentageToSteal * 100
      )}% (${oldHackThreads} threads) to ` +
        `${formatNumber(
          currentTarget.actualPercentageToSteal() * 100
        )}% (${currentTarget.getHackThreadsNeeded()} threads) ` +
        `(${currentTarget.name}) Iterations: ${attempts} Took: ${Date.now() -
          start} ms`
    )
  }
  ...
  if (
    currentTarget.percentageToSteal != 0 &&
    (currentTarget.actualPercentageToSteal() == 0 ||
      Math.abs(
        currentTarget.actualPercentageToSteal() -
          currentTarget.percentageToSteal
      ) /
        currentTarget.percentageToSteal >
        0.5)
    ...
    log(
      `WARNING: Big difference between %ToSteal (${formatNumber(
        currentTarget.percentageToSteal * 100
      )}%) ` +
        `and actual%ToSteal (${formatNumber(
          currentTarget.actualPercentageToSteal() * 100
        )}%) after ${attempts} attempts. ` +
        `Min is: ${formatNumber(
          currentTarget.percentageStolenPerHackThread() * 100
        )}%`,
      false,
      'warning'
    )
...
function analyzeSnapshot (
  ...
  const isOvershot = s =>
    !s.canBeScheduled || s.maxCompleteCycles < s.optimalPacedCycles
  if (verbose && runOnce)
    log(
      `canBeScheduled: ${snapshot.canBeScheduled},  maxCompleteCycles: ${snapshot.maxCompleteCycles}, optimalPacedCycles: ${snapshot.optimalPacedCycles}`
    )
  if (isOvershot(snapshot)) {
    return -incrementalHackThreads
  } else if (
    snapshot.maxCompleteCycles > snapshot.optimalPacedCycles &&
    lastP2steal < maxPercentageToSteal
  ) {
    // Test increasing by the increment, but if it causes us to go over maximum desired utilization, do not suggest it
    currentTarget.percentageToSteal =
      (currentTarget.getHackThreadsNeeded() + incrementalHackThreads) *
      currentTarget.percentageStolenPerHackThread()
    var comparisonSnapshot = getPerformanceSnapshot(currentTarget, networkStats)
    currentTarget.percentageToSteal = lastP2steal
    return isOvershot(comparisonSnapshot) ? 0.0 : incrementalHackThreads
  }
  return 0.0
}
...
async function performScheduling (ns, currentTarget, snapshot) {
  ...
  if (maxCycles === 0) {
    log(
      `WARNING: Attempt to schedule ${getTargetSummary(
        currentTarget
      )} returned 0 max cycles? ${JSON.stringify(snapshot)}`,
      false,
      'warning'
    )
    return
    ...
    log(
      `WARNING: Attempted to schedule empty cycle ${maxCycles} x ${getTargetSummary(
        currentTarget
  ...
  while (cyclesScheduled < maxCycles) {
    const newBatchStart = new Date(
      cyclesScheduled === 0
        ? Date.now() + queueDelay
        : lastBatch.getTime() + cycleTimingDelay
    )
    lastBatch = new Date(newBatchStart.getTime())
    const batchTiming = getScheduleTiming(newBatchStart, currentTarget)
    const newBatch = getScheduleObject(
      batchTiming,
      currentTarget,
      scheduledTasks.length
    )
    if (firstEnding === null) {
      // Can't start anything after this first hack completes (until back at min security), or we risk throwing off timing
    ...
    if (cyclesScheduled > 0 && lastStart >= firstEnding) {
      if (verbose)
        log(
          `Had to stop scheduling at ${cyclesScheduled} of ${maxCycles} desired cycles (lastStart: ${lastStart} >= firstEnding: ${firstEnding}) ${JSON.stringify(
            snapshot
          )}`
        )
      break
  ...
  for (const schedObj of scheduledTasks) {
    for (const schedItem of schedObj.scheduleItems) {
      ...
      const args = [
        ...
        schedItem.end - schedItem.start,
        discriminationArg
      ]
      if (['hack', 'grow'].includes(schedItem.toolShortName))
        // Push an arg used by remote hack/grow tools to determine whether it should manipulate the stock market
        args.push(
          stockMode &&
            ((schedItem.toolShortName == 'hack' &&
              shouldManipulateHack[currentTarget.name]) ||
              (schedItem.toolShortName == 'grow' &&
                shouldManipulateGrow[currentTarget.name]))
            ? 1
            : 0
        )
      if (['hack', 'weak'].includes(schedItem.toolShortName))
        args.push(
          options['silent-misfires'] ||
            (schedItem.toolShortName == 'hack' &&
              bitnodeMults.ScriptHackMoneyGain == 0)
            ? 1
            : 0
        ) // Optional arg to disable toast warnings about a failed hack if hacking money gain is disabled
      args.push(loopingMode ? 1 : 0) // Argument to indicate whether the cycle should loop perpetually
      if (
        recoveryThreadPadding > 1 &&
        ['weak', 'grow'].includes(schedItem.toolShortName)
      )
        schedItem.threadsNeeded *= recoveryThreadPadding // Only need to pad grow/weaken threads
      if (
        options.i &&
        currentTerminalServer?.name == currentTarget.name &&
        schedItem.toolShortName == 'hack'
      )
        schedItem.toolShortName = 'manualhack'
      const result = await arbitraryExecution(
        ns,
        getTool(schedItem.toolShortName),
        schedItem.threadsNeeded,
        args
      )
      if (result == false) {
        // If execution fails, we have probably run out of ram.
        log(
          `WARNING: Scheduling failed for ${getTargetSummary(
            currentTarget
          )} ${discriminationArg} of ${cyclesScheduled} Took: ${Date.now() -
            start}ms`,
          false,
          'warning'
        )
        currentTarget.previousCycle = `INCOMPLETE. Tried: ${cyclesScheduled} x ${getTargetSummary(
          currentTarget
        )}`
        return false
      }
  ...
  if (verbose)
    log(
      `Scheduled ${cyclesScheduled} x ${getTargetSummary(
        currentTarget
      )} Took: ${Date.now() - start}ms`
    )
  currentTarget.previousCycle = `${cyclesScheduled} x ${getTargetSummary(
    currentTarget
  )}`
  return true
}
...
function getScheduleTiming (fromDate, currentTarget) {
  ...
  const t4_secondWeakenResolvesAt = new Date(
    fromDate.getTime() + slowestTool + delayInterval * 3
  ) // step 4 - weaken after grow fires last
  const t4_fireSecondWeakenAt = new Date(
    t4_secondWeakenResolvesAt.getTime() - weakenTime
  )
  const t3_growResolvesAt = new Date(
    t4_secondWeakenResolvesAt.getTime() - delayInterval
  ...
  const t2_firstWeakenResolvesAt = new Date(
    t3_growResolvesAt.getTime() - delayInterval
  ) // step 2 (weaken after hack) should resolve "delay" before the grow.
  const t2_fireFirstWeakenAt = new Date(
    t2_firstWeakenResolvesAt.getTime() - weakenTime
  )
  const t1_hackResolvesAt = new Date(
    t2_firstWeakenResolvesAt.getTime() - delayInterval
  ) // step 1 (steal a bunch of money) should resolve "delay" before its respective weaken.
  const t1_fireHackAt = new Date(
    hackOnly ? fromDate.getTime() : t1_hackResolvesAt.getTime() - hackTime
  )
  ...
  const lastThingThatFires = new Date(
    Math.max(
      t4_fireSecondWeakenAt.getTime(),
      t3_fireGrowAt.getTime(),
      t2_fireFirstWeakenAt.getTime(),
      t1_fireHackAt.getTime()
    )
  )
  let schedule = {
    batchStart: fromDate,
  ...
  if (verbose && runOnce) {
    log(
      `Current Time: ${formatDateTime(
        new Date()
      )} Established a schedule for ${getTargetSummary(
        currentTarget
      )} from requested startTime ${formatDateTime(fromDate)}:` +
        `\n  Hack - End: ${formatDateTime(
          schedule.hackEnd
        )}  Start: ${formatDateTime(
          schedule.hackStart
        )}  Time: ${formatDuration(hackTime)}` +
        `\n  Weak1- End: ${formatDateTime(
          schedule.firstWeakenEnd
        )}  Start: ${formatDateTime(
          schedule.firstWeakenStart
        )}  Time: ${formatDuration(weakenTime)}` +
        `\n  Grow - End: ${formatDateTime(
          schedule.growEnd
        )}  Start: ${formatDateTime(
          schedule.growStart
        )}  Time: ${formatDuration(growTime)}` +
        `\n  Weak2- End: ${formatDateTime(
          schedule.secondWeakenEnd
        )}  Start: ${formatDateTime(
          schedule.secondWeakenStart
        )}  Time: ${formatDuration(weakenTime)}`
    )
  }
  return schedule
...
function getScheduleObject (batchTiming, currentTarget, batchNumber) {
  ...
  var schedHack = getScheduleItem(
    'hack',
    'hack',
    batchTiming.hackStart,
    batchTiming.hackEnd,
    currentTarget.getHackThreadsNeeded()
  )
  var schedWeak1 = getScheduleItem(
    'weak1',
    'weak',
    batchTiming.firstWeakenStart,
    batchTiming.firstWeakenEnd,
    currentTarget.getWeakenThreadsNeededAfterTheft()
  )
  ...
  if (currentTarget.percentageStolenPerHackThread() >= 1) {
    ...
    let calcThreadsForGrow = money =>
      Math.ceil(
        (
          Math.log(1 / (money / currentTarget.getMaxMoney())) /
          Math.log(currentTarget.adjustedGrowthRate()) /
          currentTarget.serverGrowthPercentage()
        ).toPrecision(14)
      )
    let stepSize = Math.floor(currentTarget.getMaxMoney() / 4),
      injectThreads = stepSize,
      schedGrowThreads = calcThreadsForGrow(injectThreads)
    for (let i = 0; i < 100 && stepSize > 0; i++) {
      if (
        injectThreads + schedGrowThreads >
        injectThreads + stepSize + calcThreadsForGrow(injectThreads + stepSize)
      )
        injectThreads += stepSize
      else if (
        injectThreads + schedGrowThreads >
    ...
    ) // Will put $injectThreads on the server
    // This will then grow from whatever % $injectThreads is back to 100%
    var schedGrow = getScheduleItem(
      'grow',
      'grow',
    ...
    var schedWeak2 = getScheduleItem(
      'weak2',
      'weak',
      ...
      Math.ceil(
        (
          ((injectThreads + schedGrowThreads) * growthThreadHardening) /
          actualWeakenPotency()
        ).toPrecision(14)
      )
    )
    if (verbose)
      log(
        `INFO: Special grow strategy since percentage stolen per hack thread is 100%: G1: ${injectThreads}, G1: ${schedGrowThreads}, W2: ${schedWeak2.threadsNeeded} (${currentTarget.name})`
      )
  } else {
    var schedGrow = getScheduleItem(
      'grow',
      ...
      batchTiming.growStart,
      batchTiming.growEnd,
      currentTarget.getGrowThreadsNeededAfterTheft()
    )
    var schedWeak2 = getScheduleItem(
      'weak2',
      'weak',
      batchTiming.secondWeakenStart,
      batchTiming.secondWeakenEnd,
      currentTarget.getWeakenThreadsNeededAfterGrowth()
    )
  }
  ...
  if (hackOnly) {
    ...
    schedItems.push(
      ...(schedHack.threadsNeeded > schedGrow.threadsNeeded
        ? [schedHack, schedGrow]
        : [schedGrow, schedHack])
    )
    ...
    schedItems.push(
      ...(schedWeak1.threadsNeeded > schedWeak2.threadsNeeded
        ? [schedWeak1, schedWeak2]
        : [schedWeak2, schedWeak1])
    )
  }
...
export async function arbitraryExecution (
  ...
  sortServerList('ram')
  var rootedServersByFreeRam = serverListByFreeRam.filter(
    server => server.hasRoot() && server.totalRam() > 1.6
  )

  // Sort servers by total ram, and try to fill these before utilizing another server.
  sortServerList('totalram')
  var preferredServerOrder = serverListByMaxRam.filter(
    server => server.hasRoot() && server.totalRam() > 1.6
  )
  ...
  var home = preferredServerOrder.splice(
  ...
  if (
    tool.shortName == 'grow' ||
    tool.shortName == 'weak' ||
    preferredServerName == 'home'
  )
    preferredServerOrder.unshift(home)
  // Send to front
  else preferredServerOrder.push(home)
  // Push all "hacknet-node" servers to the end of the preferred list, since they will lose productivity if used
  var anyHacknetNodes = []
  let hnNodeIndex
  while (
    -1 !==
    (hnNodeIndex = preferredServerOrder.indexOf(s =>
      s.name.startsWith('hacknet-node-')
    ))
  )
    anyHacknetNodes.push(preferredServerOrder.splice(hnNodeIndex, 1))
  preferredServerOrder.push(
    ...anyHacknetNodes.sort((a, b) =>
      b.totalRam != a.totalRam
        ? b.totalRam - a.totalRam
  ...
  if (
    preferredServerName &&
    preferredServerName != 'home' /*home is handled above*/
  ) {
    const preferredServerIndex = preferredServerOrder.findIndex(
      i => i.name == preferredServerName
    )
    if (preferredServerIndex != -1)
      preferredServerOrder.unshift(
        preferredServerOrder.splice(preferredServerIndex, 1)[0]
      )
    else
      log(
        `ERROR: Configured preferred server "${preferredServerName}" for ${tool.name} is not a valid server name`,
        true,
        'error'
      )
  }
  ...
  for (
    var i = 0;
    i < rootedServersByFreeRam.length && remainingThreads > 0;
    i++
  ) {
    var targetServer = rootedServersByFreeRam[i]
    var maxThreadsHere = Math.min(
      remainingThreads,
      computeMaxThreads(targetServer)
    )
    ...
    if (
      targetServer.name != daemonHost &&
      !doesFileExist(tool.name, targetServer.name)
    ) {
      if (verbose)
        log(
          `Copying ${tool.name} from ${daemonHost} to ${targetServer.name} so that it can be executed remotely.`
        )
  ...
  if (remainingThreads > 0)
    log(
      `ERROR: Ran out of RAM to run ${tool.name} against ${
        args[0]
      } - ${threads - remainingThreads} of ${threads} threads were spawned.`,
      false,
      'error'
    )
...
async function prepServer (ns, currentTarget) {
  ...
  if (
    currentTarget.getMoney() < currentTarget.getMaxMoney() &&
    !stockFocus /* Prep should only weaken in stock-focus mode */
  ) {
    ...
    weakenForGrowthThreadsNeeded = Math.ceil(
      (
        (growThreadsScheduled * growthThreadHardening) /
        actualWeakenPotency()
      ).toPrecision(14)
    )
    ...
    if (growThreadsScheduled > 0)
      prepSucceeding = await arbitraryExecution(
        ns,
        growTool,
        growThreadsScheduled,
        [currentTarget.name, now.getTime(), now.getTime(), 0, 'prep']
      )
  ...
  if (
    prepSucceeding &&
    (currentTarget.getSecurity() > currentTarget.getMinSecurity() ||
      weakenForGrowthThreadsNeeded > 0)
  ) {
    ...
    weakenThreadsScheduled = Math.min(
      weakenThreadsAllowable,
      weakenThreadsNeeded
    )
  ...
  if (
    verbose &&
    prepSucceeding &&
    (weakenThreadsScheduled > 0 || growThreadsScheduled > 0)
  )
    log(
      `Prepping with ${weakenThreadsScheduled} weaken, ${growThreadsScheduled} grow threads (${weakenThreadsNeeded ||
        0} / ${growThreadsNeeded || 0} needed)` +
        ' ETA ' +
        Math.floor((currentTarget.timeToWeaken() + queueDelay) / 1000) +
        's (' +
        currentTarget.name +
        ')' +
        ' Took: ' +
...
function getXPFarmServer (all = false) {
  const hackableServers = serverListByMaxRam
    .filter(
      server =>
...
async function kickstartHackXp (
  ns,
  percentOfFreeRamToConsume = 1,
  verbose = false,
  targets = undefined
) {
  if (!xpOnly)
    return await scheduleHackExpCycle(
      ns,
      getXPFarmServer(),
      percentOfFreeRamToConsume,
      verbose,
      false
    ) // Grind some XP from the single best target for farming XP
  ...
  var jobHosts = serverListByMaxRam.filter(
    s => s.hasRoot() && s.totalRam() > 128
  ) // Get the set of servers that can be reasonably expected to host decent-sized jobs
  if (jobHosts.length == 0)
    jobHosts = serverListByMaxRam.filter(s => s.hasRoot() && s.totalRam() > 16) // Lower our standards if we're early-game and nothing qualifies
  ...
  targets = Math.min(
    maxTargets,
    Math.floor(jobHosts.filter(s => s.totalRam() > 0.01 * homeRam).length)
  ) // Limit targets (too many creates lag which worsens performance, and need a dedicated server for each)
  let targetsByExp = getXPFarmServer(true)
  if (options.i) {
    // To farm intelligence, use manual hack on only the current connected server
  ...
  if (totalServerRam > lastCycleTotalRam) {
    // If we've aqcuired more ram, remove restrictions and discover the new best balance
    singleServerLimit = 0
...
async function waitForCycleEnd (
  ns,
  server,
  maxWaitTime = 200,
  waitInterval = 5
  ...
  if (verbose)
    return log(
      `WARNING: ${server.name} FarmXP process is still in progress from a prior run. Completion time is unknown...`
    )
  const activeCycleTimeLeft = (eta || 0) - Date.now()
  let stillBusy
  if (verbose)
    log(
      `Waiting for last ${server.name} FarmXP process to complete... (ETA ${
        eta ? formatDuration(activeCycleTimeLeft) : 'unknown'
      })`
    )
  ...
  if (stillBusy)
    log(
      `WARNING: ${server.name} FarmXP process is ` +
        (eta
          ? `more than ${formatDuration(-activeCycleTimeLeft)} overdue...`
          : 'still in progress from a prior run...')
    )
  return !stillBusy
}
...
/** @param {NS} ns **/
async function scheduleHackExpCycle (
  ns,
  server,
  percentOfFreeRamToConsume,
  verbose,
  ...
  if (!server.hasRoot())
    return log(
      `ERROR: Cannot farm XP from unrooted server ${server.name}`,
      true,
      'error'
    )
  ...
  try {
    ...
    if (advancedMode) {
      // We get the most XP by using max possible hack threads while keeping money just above 0 (so that we get full hack() exp)
      expTool = options.i ? getTool('manualhack') : getTool('hack')
      expTime = server.timeToHack()
    } else if (server.getSecurity() > server.getMinSecurity()) {
      // If the server isn't at min-security, we should do that (to reduce hack/grow/weaken time to the minimum)
      expTool = getTool('weak')
      expTime = server.timeToWeaken()
    } else {
      // If the server is at min-security, we should farm grow(), since it takes less time (80%) than weaken(). Once at max-money, grow will no longer reduce security.
      expTool = getTool('grow')
      expTime = server.timeToGrow()
    }
    if (server.isXpFarming()) {
      if (verbose && activeCycleTimeLeft < -50)
        // Warn about big misfires (sign of lag)
        log(
          `WARNING: ${server.name} FarmXP process is ` +
            (eta
              ? `more than ${formatDuration(-activeCycleTimeLeft)} overdue...`
              : `still in progress from a prior run. ETA unknown, assuming '${
                  expTool.name
                }' time: ${formatDuration(expTime)}`)
        )
      return eta
        ? activeCycleTimeLeft > 0
          ? activeCycleTimeLeft
          : 10 /* If we're overdue, sleep only 10 ms before checking again */
        : expTime /* Have no ETA, sleep for expTime */
    }
    let threads = Math.floor(
      (
        (allocatedServer == null
          ? expTool.getMaxThreads()
          : allocatedServer.ramAvailable() / expTool.cost) *
        percentOfFreeRamToConsume
      ).toPrecision(14)
    )
    if (threads == 0)
      return log(
        `WARNING: Cannot farm XP from ${server.name}, threads == 0 for allocated server ` +
          (allocatedServer == null
            ? '(any server)'
            : `${allocatedServer.name} with ${formatRam(
                allocatedServer.ramAvailable()
              )} free RAM`),
        false,
        'warning'
      )
    ...
    if (advancedMode) {
      // Need to keep server money above zero, and security at minimum to farm xp from hack();
      const effectiveHackThreads = Math.ceil(
        1 / server.percentageStolenPerHackThread()
      ) // Only this many hack threads "count" for stealing/hardening. The rest get a 'free ride'
      ...
      const securityHardeningToCombat = Math.max(
        effectiveHackThreads * hackThreadHardening +
          growThreadsNeeded * growthThreadHardening, // Security that will be incurred hack() + grow() threads
        server.getSecurity() - server.getMinSecurity()
      ) // If the current security level is higher than this, add enough weaken threads to correct it
      var weakenThreadsNeeded = Math.ceil(
        securityHardeningToCombat / actualWeakenPotency()
      )
      ...
      if (singleServer)
        // If set to only use a single server, free up the hack threads to make room for recovery threads
        threads = Math.max(
          0,
          threads -
            Math.ceil(
              ((growThreadsNeeded + weakenThreadsNeeded) * 1.75) / expTool.cost
            )
        ) // Make room for recovery threads
      if (threads == 0)
        return log(
          `Cannot farm XP from ${server.name} on ` +
            (allocatedServer == null
              ? '(any server)'
              : `${allocatedServer.name} with ${formatRam(
                  allocatedServer.ramAvailable()
    ...
    let success = await arbitraryExecution(
      ns,
      expTool,
      threads,
      [server.name, scheduleTime, 0, expTime, 'FarmXP'],
    ...
    if (advancedMode) {
      ...
      success &&= await arbitraryExecution(
        ns,
        getTool('grow'),
        growThreadsNeeded,
        [server.name, scheduleGrow, 0, server.timeToGrow(), 'growForXp'],
        singleServer ? allocatedServer?.name : null,
        !singleServer
      )
      success &&= await arbitraryExecution(
        ns,
        getTool('weak'),
        weakenThreadsNeeded,
        [server.name, scheduleWeak, 0, server.timeToWeaken(), 'weakenForXp'],
        singleServer ? allocatedServer?.name : null,
        !singleServer
      )
      ...
      if (verbose)
        log(
          `Exp Cycle: ${threads} x Hack in ${Math.round(
            scheduleTime - now + expTime
          )}ms, ` +
            `${growThreadsNeeded} x Grow in ${Math.round(
              (scheduleGrow - now + server.timeToGrow()) % cycleTime
            )}ms, ` +
            `${weakenThreadsNeeded} x Weak in ${Math.round(
              (scheduleWeak - now + server.timeToWeaken()) % cycleTime
            )}ms, ` +
            `Tick: ${Math.round(cycleTime)}ms on ${allocatedServer?.name ??
              '(any server)'} targeting "${server.name}"`
        )
    } else if (verbose)
      log(
        `In ${formatDuration(cycleTime)}, ${threads} ${
          expTool.shortName
        } threads will fire against ${server.name} (for Hack Exp)`
      )
    if (!success) {
      // If some aspect scheduling fails, we should try adjusting our future scheduling tactics to attempt to use less RAM
      if (singleServerLimit >= maxTargets && maxTargets > 1) maxTargets--
      else singleServerLimit++
    }
...
async function updateStockPositions (ns) {
  ...
  if (!updatedPositions) {
    failedStockUpdates++
    if (failedStockUpdates % 60 == 10)
      // Periodically warn if stockmaster is not running (or not generating the required file)
      log(
        `WARNING: The file "/Temp/stock-probabilities.txt" has been missing or empty the last ${failedStockUpdates} attempts.` +
          `\nEnsure stockmaster.js is running, or turn off the --stock-manipulation flag when running.`,
        false,
        'warning'
      )
    return
  ...
  const newShouldManipulateGrow = {},
    newShouldManipulateHack = {},
    newServersWithOwnedStock = []
  Object.keys(serverStockSymbols).forEach(server => {
    const sym = serverStockSymbols[server]
    const pos = updatedPositions[sym]
    newShouldManipulateGrow[server] =
      pos.sharesLong > 0 ? true : pos.prob >= 0.5 // If bullish, grow should be made to influence stock
    newShouldManipulateHack[server] =
      pos.sharesShort > 0 ? true : pos.prob < 0.5 // If bearish, hack should be made to influence stock
    if (pos.sharesLong > 0 || pos.sharesShort > 0)
      newServersWithOwnedStock.push(server) // Keep track of servers we own stock in so we can prioritize hacking them in stockFocus mode
  })
  if (stockFocus) {
    // Detect any positions that have reversed and kill all active hack/grow scripts against that server set to manipulate in the wrong direction
    const newLongPositions = Object.keys(serverStockSymbols).filter(
      server => newShouldManipulateGrow[server] && !shouldManipulateGrow[server]
    )
    if (newLongPositions.length > 0)
      await terminateScriptsManipulatingStock(
        ns,
        newLongPositions,
        getTool('hack').name
      ) // Make sure no hacks are set to manipulate our long positions down!
    const newShortPositions = Object.keys(serverStockSymbols).filter(
      server => newShouldManipulateHack[server] && !shouldManipulateHack[server]
    )
    if (newShortPositions.length > 0)
      await terminateScriptsManipulatingStock(
        ns,
        newShortPositions,
        getTool('grow').name
      ) // Make sure no grows are set to manipulate our short positions up!
  }
...
async function terminateScriptsManipulatingStock (ns, servers, toolName) {
  const problematicProcesses = addedServerNames.flatMap(hostname =>
    ps(ns, hostname)
      .filter(
        process =>
          servers.includes(process.args[0]) &&
          (loopingMode ||
            (toolName == process.filename &&
              process.args.length > 5 &&
              process.args[5]))
      )
  ...
  if (problematicProcesses.length > 0)
    await runCommand(
      ns,
      JSON.stringify(problematicProcesses) +
        '.forEach(p => ns.kill(p.pid, p.hostname))',
      '/Temp/kill-remote-stock-manipulation.js'
    )
}
...
function removeServerByName (deletedHostName) {
  ...
  const removeByName = (hostname, list, listname) => {
    ...
    if (toRemove === -1)
      ...
      log(
        `${hostname} was found at index ${toRemove} of list ${listname} and removed leaving ${list.length} items.`
      )
    }
  }
...
let getServerByName = hostname =>
  serverListByFreeRam.find(s => s.name == hostname)
...
let isFlaggedForDeletion = hostName =>
  hostName != 'home' && doesFileExist('/Flags/deleting.txt', hostName)
...
function buildServerList (ns, verbose = false) {
  ...
  let allServers = scanAllServers(ns).filter(
    hostName => !isFlaggedForDeletion(hostName)
  )
  ...
  if (!useHacknetNodes)
    allServers = allServers.filter(
      hostName => !hostName.startsWith('hacknet-node-')
    )
  // Remove all servers we currently have added that are no longer being returned by the above query
  for (const hostName of addedServerNames.filter(
    hostName => !allServers.includes(hostName)
  ))
    removeServerByName(hostName)
  // Add any servers that are new
  allServers
    .filter(hostName => !addedServerNames.includes(hostName))
    .forEach(hostName => addServer(buildServerObject(ns, hostName, verbose)))
}
  ...
  switch (o) {
    ...
    case 'targeting':
      ...
      serverListByTargetOrder.sort(function (a, b) {
        ...
        if (stockFocus) {
          // If focused on stock-market manipulation, sort up servers with a stock, prioritizing those we have some position in
          let stkCmp =
            serversWithOwnedStock.includes(a.name) ==
            serversWithOwnedStock.includes(b.name)
              ? 0
              : serversWithOwnedStock.includes(a.name)
              ? -1
              : 1
          if (stkCmp == 0)
            stkCmp =
              (shouldManipulateGrow[a.name] || shouldManipulateHack[a.name]) ==
              (shouldManipulateGrow[b.name] || shouldManipulateHack[b.name])
                ? 0
                : shouldManipulateGrow[a.name] || shouldManipulateHack[a.name]
                ? -1
                : 1
        ...
        if (
          (a.isPrepped() || a.isTargeting()) != (b.isPrepped() || b.isTargeting)
        )
          return a.isPrepped() || a.isTargeting() ? -1 : 1
...
async function getNsDataThroughFile (ns, ...args) {
  return await getNsDataThroughFile_Custom(
    ns,
    getFnRunViaNsExec(ns, daemonHost),
    getFnIsAliveViaNsPs(ns),
    ...args
  )
}
...
async function establishMultipliers (ns) {
  ...
  bitnodeMults = (await tryGetBitNodeMultipliers_Custom(
    ns,
...
function buildToolkit (ns) {
  log('buildToolkit')
  for (const toolConfig of hackTools
...
function getTool (s) {
  return (
    toolsByShortName[s] ||
    toolsByShortName[s.shortName || hashToolDefinition(s)]
  )
}
...
function doRoot (server) {
  try {
    ...
    log(
      `ERROR while attempting to root ${server.name} with ${server.portsRequired} ports needed.`,
      true,
      'error'
    )
    throw err
