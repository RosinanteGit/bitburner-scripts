...
export async function main (ns) {
  ...
  let purchasedServers = await getNsDataThroughFile(
    ns,
    'ns.getPurchasedServers()',
    '/Temp/purchased-servers.txt'
  )
  ...
  if (worstServerName == null) {
    ns.tprint(
      'Nothing to delete - all ' +
        purchasedServers.length +
        ' servers have the maximum ' +
        worstServerRam +
        ' GB of RAM'
    )
    return
  ...
  await runCommand(
    ns,
    `await ns.scp("/Flags/deleting.txt", "${worstServerName}")`,
    '/Temp/flag-server-for-deletion.js'
  )
  var success = await getNsDataThroughFile(
    ns,
    `ns.deleteServer("${worstServerName}")`,
    '/Temp/try-delete-server-result.txt'
  )
  if (success)
    ns.tprint(
      'Deleted ' +
    ...
    ns.tprint(
      'Tried to delete ' +
        ...
        ' with ' +
        worstServerRam +
        ' GB RAM, but it failed (scripts still running)'
    )
}
