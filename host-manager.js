...
export async function main (ns) {
  ...
  maxPurchasableServerRamExponent = Math.round(
    20 + Math.log2(bitnodeMults.PurchasedServerMaxRam)
  )
  ...
  if (!keepRunning)
    ns.print(
      `host-manager will run once. Run with argument "-c" to run continuously.`
    )
  do {
...
function tryToBuyBestServerPossible (ns) {
  ...
  while (hostsToScan.length > 0 && infLoopProtection-- > 0) {
    ...
    if (
      ...
      ns.hasRootAccess(hostName) == false
    ) {
  ...
  //let purchasedServers = ns.getPurchasedServers();
  let purchasedServers = rootedServers
    .filter(hostName => hostName.startsWith(purchasedServerName))
    .sort()

  // analyze the utilization rates
  let utilizationRate = utilizationTotal / totalMaxRam
  setStatus(
    `Using ${Math.round(utilizationTotal).toLocaleString()}/${formatRam(
      totalMaxRam
    )} (` +
      `${(utilizationRate * 100).toFixed(1)}%) across ${
        rootedServers.length
      } servers (${purchasedServers.length} bought)`
  )

  // Stop if utilization is below target. We probably don't need another server.
  if (utilizationRate < utilizationTarget) return
  ...
  spendableMoney = Math.max(
    0,
    Math.min(
      spendableMoney * (1 - reservedMoneyPercent),
      spendableMoney - reservedMoneyAmount
    )
  )
  if (spendableMoney == 0)
  ...
  for (; exponentLevel < maxPurchasableServerRamExponent; exponentLevel++)
    if (
      ns.getPurchasedServerCost(Math.pow(2, exponentLevel + 1)) > spendableMoney
    )
      break
  ...
  if (exponentLevel < minRamExponent)
    return setStatus(
      `${prefix}The highest ram exponent we can afford (2^${exponentLevel} for ${formatMoney(
        cost
      )}) on our budget of ${formatMoney(spendableMoney)} ` +
  ...
  if (exponentLevel < maxPurchasableServerRamExponent) {
    ...
    if (maxRamPossibleToBuy < ns.getServerMaxRam('home') / 4)
      return setStatus(
        prefix +
          'the most RAM we can buy (' +
          formatRam(maxRamPossibleToBuy) +
          ') is way less than (<0.25*) home RAM ' +
          formatRam(ns.getServerMaxRam('home'))
      )
    ...
    if (maxRamPossibleToBuy / totalMaxRam < 0.1)
      return setStatus(
        prefix +
          'the most RAM we can buy (' +
          formatRam(maxRamPossibleToBuy) +
          ') is less than 10% of total available RAM ' +
  ...
  if (worstServerName != null && maxRamPossibleToBuy < worstServerRam)
    return setStatus(
      prefix +
        'the most RAM we can buy (' +
        formatRam(maxRamPossibleToBuy) +
        ') is less than our worst purchased server ' +
        worstServerName +
        "'s RAM " +
        formatRam(worstServerRam)
    )
  ...
  if (bestServerRam != null && maxRamPossibleToBuy < bestServerRam)
    return setStatus(
      prefix +
        'the most RAM we can buy (' +
        formatRam(maxRamPossibleToBuy) +
        ') is less than our previously purchased server ' +
        bestServerName +
        ' RAM ' +
        formatRam(bestServerRam)
    )
  ...
  if (purchasedServers.length >= maxPurchasedServers) {
    ...
    if (
      exponentLevel == maxPurchasableServerRamExponent ||
      worstServerRam * 16 <= maxRamPossibleToBuy
    ) {
      ns.run('remove-worst-server.js')
      return setStatus(
        `hostmanager.js requested to delete server ${worstServerName} (${formatRam(
          worstServerRam
        )} RAM) ` +
      ...
      return setStatus(
        `${prefix}the most RAM we can buy (${formatRam(
          maxRamPossibleToBuy
        )}) is less than 16x the RAM ` +
          `of the server it must delete to make room: ${worstServerName} (${formatRam(
            worstServerRam
          )} RAM)`
      )
    }
  ...
  let purchasedServer = ns.purchaseServer(
    purchasedServerName,
    maxRamPossibleToBuy
  )
  if (!purchasedServer)
    setStatus(
      prefix +
        `Could not purchase a server with ${formatRam(
          maxRamPossibleToBuy
        )} RAM for ${formatMoney(cost)} ` +
        `with a budget of ${formatMoney(
          spendableMoney
        )}. This is either a bug, or we in a SF.9`
    )
  else
    announce(
